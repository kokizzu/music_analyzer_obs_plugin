...
void check_quiet_monophonic_other_recovery_bounds(Runner &runner)
{
	...
	runner.expect(is_quiet_named_string_g4_recovery_candidate("string track", 67, 0.0027f, 2.05f),
		      "quiet named string G4 recovery: expected measured direct tail to remain eligible");
	runner.expect(!is_quiet_named_string_g4_recovery_candidate("wind track", 67, 0.0027f, 2.05f),
		      "quiet named string G4 recovery: expected non-string tail to stay rejected");
	runner.expect(!is_quiet_named_string_g4_recovery_candidate("string track", 66, 0.0027f, 2.05f),
		      "quiet named string G4 recovery: expected neighboring pitch to stay rejected");
	runner.expect(!is_quiet_named_string_g4_recovery_candidate("string track", 67, 0.0025f, 2.05f),
		      "quiet named string G4 recovery: expected sub-floor tail to stay rejected");
	runner.expect(!is_quiet_named_string_g4_recovery_candidate("string track", 67, 0.0027f, 1.99f),
		      "quiet named string G4 recovery: expected weak direct body to stay rejected");
}
...
void check_supported_low_monophonic_other_fundamental(Runner &runner)
{
	...
	powers.fill(0.0f);
	set_probe_level(powers, 58, 0.080f); // A#3 compact Surprise body
	set_probe_level(powers, 70, 1.00f); // A#4 selected octave
	set_probe_level(powers, 77, 0.55f); // F5 compact fifth
	set_probe_level(powers, 82, 0.24f); // A#5 second octave
	set_probe_level(powers, 86, 0.14f); // C6 upper major third
	set_probe_level(powers, 89, 0.040f); // F6 upper fifth
	runner.expect(supported_named_brass_rich_octave_fundamental("brass track", powers, 70) == 58,
		      "named brass octave recovery: expected bounded compact A#3 ladder");
	set_probe_level(powers, 77, 0.70f);
	runner.expect(supported_named_brass_rich_octave_fundamental("brass track", powers, 70) < 0,
	...
	powers.fill(0.0f);
	set_probe_level(powers, 58, 0.024f); // A#3 direct trumpet body
	set_probe_level(powers, 70, 1.00f); // A#4 selected octave
	set_probe_level(powers, 77, 0.332f); // F5 fifth
	set_probe_level(powers, 82, 0.082f); // A#5 second octave
	set_probe_level(powers, 86, 0.111f); // D6 upper major third
	set_probe_level(powers, 89, 0.004f); // F6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) == 58,
		      "low monophonic other recovery: expected bounded A#3 trumpet tiny-body octave stack");
	set_probe_level(powers, 86, 0.125f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) != 58,
		      "low monophonic other recovery: expected bright A#3 trumpet upper third to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 62, 0.218f); // D4 direct viola body
	set_probe_level(powers, 74, 0.100f); // D5 octave
	set_probe_level(powers, 81, 0.122f); // A5 fifth
	set_probe_level(powers, 86, 1.00f); // D6 selected second octave
	set_probe_level(powers, 90, 0.049f); // F#6 upper major third
	set_probe_level(powers, 93, 0.171f); // A6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 86) == 62,
		      "low monophonic other recovery: expected bounded D4 viola compact second-octave stack");
	set_probe_level(powers, 81, 0.14f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 86) != 62,
		      "low monophonic other recovery: expected bright D4 viola fifth to stay rejected");
	powers.fill(0.0f);
	...
	set_probe_level(powers, 69, 1.00f);  // A4 selected fifth
	set_probe_level(powers, 74, 0.65f);  // D5 second octave
	set_probe_level(powers, 78, 0.70f);  // F#5 upper major third
	set_probe_level(powers, 81, 0.66f);  // A5 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 69) == 50,
		      "low monophonic other recovery: expected bounded D3 trombone rich fifth stack");
	set_probe_level(powers, 62, 0.249f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 69) != 50,
		      "low monophonic other recovery: expected sub-boundary D3 trombone octave to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 46, 0.142f); // A#2 direct trombone body
	set_probe_level(powers, 58, 0.038f); // A#3 faint octave
	set_probe_level(powers, 65, 0.210f); // F4 fifth
	set_probe_level(powers, 70, 1.00f);  // A#4 selected second octave
	set_probe_level(powers, 74, 0.52f);  // D5 upper major third
	set_probe_level(powers, 77, 0.36f);  // F5 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) == 46,
		      "low monophonic other recovery: expected bounded A#2 trombone second-octave stack");
	set_probe_level(powers, 46, 0.134f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) != 46,
	...
	set_probe_level(powers, 77, 1.00f); // F5 selected fifth
	set_probe_level(powers, 82, 0.06f); // A#5 second octave
	set_probe_level(powers, 86, 0.25f); // C6 upper major third
	set_probe_level(powers, 89, 0.40f); // F6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 77) == 58,
		      "low monophonic other recovery: expected bounded A#3 clarinet fifth stack");
	set_probe_level(powers, 70, 0.08f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 77) != 58,
		      "low monophonic other recovery: expected bright A#3 clarinet octave to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 59, 0.095f); // B3 direct trombone body
	set_probe_level(powers, 71, 1.00f); // B4 selected octave
	set_probe_level(powers, 78, 0.60f); // F#5 broad fifth
	set_probe_level(powers, 83, 0.20f); // B5 second octave
	set_probe_level(powers, 87, 0.040f); // D#6 upper major third
	set_probe_level(powers, 90, 0.060f); // F#6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 71) == 59,
		      "low monophonic other recovery: expected bounded B3 trombone octave stack");
	set_probe_level(powers, 78, 0.75f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 71) != 59,
		      "low monophonic other recovery: expected over-bright B3 trombone fifth to stay rejected");
	powers.fill(0.0f);
	...
	set_probe_level(powers, 78, 0.05f); // F#5 quiet second octave
	set_probe_level(powers, 82, 0.09f); // A#5 upper major third
	set_probe_level(powers, 85, 0.04f); // C#6 quiet upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 66) == 54,
		      "low monophonic other recovery: expected bounded F#3 bassoon boundary stack");
	set_probe_level(powers, 78, 0.09f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 66) != 54,
		      "low monophonic other recovery: expected bright F#3 second octave to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 54, 0.10f); // F#3 bright-third bassoon body
	set_probe_level(powers, 66, 1.00f); // F#4 selected octave
	set_probe_level(powers, 73, 0.37f); // C#5 fifth
	set_probe_level(powers, 78, 0.04f); // F#5 selected second octave
	set_probe_level(powers, 82, 0.20f); // A#5 bright upper major third
	set_probe_level(powers, 85, 0.07f); // C#6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 66) == 54,
		      "low monophonic other recovery: expected bounded bright-third F#3 bassoon stack");
	set_probe_level(powers, 85, 0.11f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 66) != 54,
		      "low monophonic other recovery: expected over-bright F#3 upper fifth to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 69, 0.10f); // A4 sparse bassoon body
	set_probe_level(powers, 81, 1.00f); // A5 selected octave
	set_probe_level(powers, 88, 0.08f); // E6 small fifth
	set_probe_level(powers, 93, 0.01f); // A6 nearly absent second octave
	set_probe_level(powers, 97, 0.003f); // C#7 quiet upper major third
	set_probe_level(powers, 100, 0.001f); // E7 quiet upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 81) == 69,
		      "low monophonic other recovery: expected bounded sparse A4 bassoon octave stack");
	set_probe_level(powers, 93, 0.012f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 81) != 69,
		      "low monophonic other recovery: expected bright A4 second octave to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 59, 0.18f); // B3 compact bassoon body
	set_probe_level(powers, 71, 1.00f); // B4 selected octave
	set_probe_level(powers, 78, 0.19f); // F#5 compact fifth
	set_probe_level(powers, 83, 0.10f); // B5 compact second octave
	set_probe_level(powers, 87, 0.04f); // D#6 quiet upper major third
	set_probe_level(powers, 90, 0.01f); // F#6 quiet upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 71) == 59,
		      "low monophonic other recovery: expected bounded compact B3 bassoon octave stack");
	set_probe_level(powers, 78, 0.23f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 71) != 59,
		      "low monophonic other recovery: expected over-bright B3 fifth to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 55, 0.16f); // G3 boundary bassoon body
	set_probe_level(powers, 67, 1.00f); // G4 selected octave
	set_probe_level(powers, 74, 0.28f); // D5 boundary fifth
	set_probe_level(powers, 79, 0.03f); // G5 compact second octave
	set_probe_level(powers, 83, 0.22f); // B5 upper major third
	set_probe_level(powers, 86, 0.07f); // D6 bounded upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) == 55,
		      "low monophonic other recovery: expected bounded G3 bassoon boundary stack");
	set_probe_level(powers, 86, 0.09f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) != 55,
		      "low monophonic other recovery: expected bright G3 upper fifth to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 55, 0.085f); // G3 quiet violin body
	set_probe_level(powers, 67, 0.80f);  // G4 substantial octave
	set_probe_level(powers, 74, 1.00f);  // D5 selected fifth
	set_probe_level(powers, 79, 0.25f);  // G5 second octave
	set_probe_level(powers, 83, 0.34f);  // B5 upper major third
	set_probe_level(powers, 86, 0.50f);  // D6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 74) == 55,
		      "low monophonic other recovery: expected bounded quiet G3 violin fifth stack");
	set_probe_level(powers, 86, 0.57f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 74) != 55,
		      "low monophonic other recovery: expected over-rich G3 violin fifth stack to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 55, 0.025f); // G3 ultra-weak violin body
	set_probe_level(powers, 67, 1.00f); // G4 selected octave
	set_probe_level(powers, 74, 0.52f); // D5 bounded fifth
	set_probe_level(powers, 79, 0.13f); // G5 compact second octave
	set_probe_level(powers, 83, 0.50f); // B5 upper major third
	set_probe_level(powers, 86, 0.10f); // D6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) == 55,
		      "low monophonic other recovery: expected bounded ultra-weak G3 violin octave stack");
	set_probe_level(powers, 86, 0.14f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) != 55,
		      "low monophonic other recovery: expected bright G3 ultra-weak upper fifth to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 55, 0.024f); // G3 forte violin body
	set_probe_level(powers, 67, 1.00f); // G4 selected octave
	set_probe_level(powers, 74, 0.68f); // D5 strong fifth
	set_probe_level(powers, 79, 0.21f); // G5 second octave
	set_probe_level(powers, 83, 0.40f); // B5 upper major third
	set_probe_level(powers, 86, 0.18f); // D6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) == 55,
		      "low monophonic other recovery: expected bounded forte G3 violin fifth ladder");
	set_probe_level(powers, 83, 0.44f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) != 55,
		      "low monophonic other recovery: expected bright forte G3 upper third to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 55, 0.025f); // G3 even forte violin body
	set_probe_level(powers, 67, 1.00f); // G4 selected octave
	set_probe_level(powers, 74, 0.62f); // D5 even fifth
	set_probe_level(powers, 79, 0.13f); // G5 compact second octave
	set_probe_level(powers, 83, 0.43f); // B5 bounded upper major third
	set_probe_level(powers, 86, 0.07f); // D6 bounded upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) == 55,
		      "low monophonic other recovery: expected bounded even forte G3 violin ladder");
	set_probe_level(powers, 79, 0.17f);
	set_probe_level(powers, 83, 0.44f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) != 55,
		      "low monophonic other recovery: expected out-of-range even forte G3 ladder to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 55, 0.024f); // G3 mezzo-forte violin body
	set_probe_level(powers, 67, 1.00f); // G4 selected octave
	set_probe_level(powers, 74, 0.55f); // D5 bounded fifth
	set_probe_level(powers, 79, 0.13f); // G5 compact second octave
	set_probe_level(powers, 83, 0.24f); // B5 low upper major third
	set_probe_level(powers, 86, 0.06f); // D6 bounded upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) == 55,
		      "low monophonic other recovery: expected bounded mezzo-forte G3 violin ladder");
	set_probe_level(powers, 79, 0.19f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) != 55,
	...
	set_probe_level(powers, 82, 0.18f);  // A#5 second octave
	set_probe_level(powers, 86, 0.05f);  // C6 upper major third
	set_probe_level(powers, 89, 0.007f); // F6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) == 58,
		      "low monophonic other recovery: expected bounded sparse A#3 violin octave stack");
	set_probe_level(powers, 89, 0.012f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) != 58,
		      "low monophonic other recovery: expected over-bright A#3 violin upper fifth to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 70, 0.11f);  // A#4 oboe body
	set_probe_level(powers, 82, 0.15f);  // A#5 modest octave
	set_probe_level(powers, 89, 1.00f);  // F6 selected fifth
	set_probe_level(powers, 94, 0.055f); // A#6 compact second octave
	set_probe_level(powers, 98, 0.008f); // C7 upper major third
	set_probe_level(powers, 101, 0.004f); // F7 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 89) == 70,
		      "low monophonic other recovery: expected bounded sparse A#4 oboe fifth stack");
	...
	powers.fill(0.0f);
	set_probe_level(powers, 58, 0.07f); // A#3 mezzo-forte violin body
	set_probe_level(powers, 70, 1.00f); // A#4 selected octave
	set_probe_level(powers, 77, 0.018f); // F5 sparse fifth
	set_probe_level(powers, 82, 0.17f); // A#5 compact second octave
	set_probe_level(powers, 86, 0.04f); // C6 restrained upper major third
	set_probe_level(powers, 89, 0.01f); // F6 bounded upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) == 58,
		      "low monophonic other recovery: expected bounded mezzo-forte A#3 violin octave stack");
	set_probe_level(powers, 77, 0.031f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 70) != 58,
		      "low monophonic other recovery: expected bright A#3 violin fifth to stay rejected");
	powers.fill(0.0f);
	...
	set_probe_level(powers, 80, 0.17f); // G#5 second octave
	set_probe_level(powers, 84, 0.36f); // B5 upper major third
	set_probe_level(powers, 87, 0.06f); // D#6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 68) == 56,
		      "low monophonic other recovery: expected bounded rich G#3 violin octave stack");
	set_probe_level(powers, 87, 0.10f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 68) != 56,
		      "low monophonic other recovery: expected bright G#3 violin upper fifth to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 56, 0.068f); // G#3 compact violin body
	set_probe_level(powers, 68, 1.00f); // G#4 selected octave
	set_probe_level(powers, 75, 0.24f); // D#5 fifth
	set_probe_level(powers, 80, 0.25f); // G#5 compact second octave
	set_probe_level(powers, 84, 0.34f); // B5 rich upper major third
	set_probe_level(powers, 87, 0.06f); // D#6 bounded upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 68) == 56,
		      "low monophonic other recovery: expected compact G#3 violin upper ladder");
	set_probe_level(powers, 56, 0.071f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 68) != 56,
		      "low monophonic other recovery: expected strong G#3 body to stay rejected");
	powers.fill(0.0f);
	set_probe_level(powers, 53, 0.10f); // F3 viola body
	set_probe_level(powers, 65, 1.00f); // F4 selected octave
	set_probe_level(powers, 72, 0.15f); // C5 faint fifth
	set_probe_level(powers, 77, 0.10f); // F5 second octave
	set_probe_level(powers, 81, 0.02f); // A5 upper major third
	set_probe_level(powers, 84, 0.03f); // C6 upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 65) == 53,
		      "low monophonic other recovery: expected bounded F3 viola octave stack");
	set_probe_level(powers, 72, 0.22f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 65) != 53,
	...
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) == 55,
		      "low monophonic other recovery: expected bounded G3 octave-only wind profile");
	set_probe_level(powers, 55, 0.12f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 67) != 55,
		      "low monophonic other recovery: expected weak G3 octave-only body to stay rejected");
	powers.fill(0.0f);
	...
	powers.fill(0.0f);
	set_probe_level(powers, 77, 0.13f);  // F5 direct oboe body
	set_probe_level(powers, 89, 1.00f);  // F6 selected octave
	set_probe_level(powers, 96, 0.011f); // C6 restrained fifth
	set_probe_level(powers, 101, 0.05f); // F7 compact second octave
	set_probe_level(powers, 105, 0.001f); // A7 quiet upper major third
	set_probe_level(powers, 108, 0.02f); // C8 bounded upper fifth
	runner.expect(supported_low_monophonic_other_fundamental(powers, 89) == 77,
		      "low monophonic other recovery: expected bounded direct F5 oboe octave stack");
	set_probe_level(powers, 108, 0.032f);
	runner.expect(supported_low_monophonic_other_fundamental(powers, 89) != 77,
		      "low monophonic other recovery: expected over-bright F5 oboe upper fifth to stay rejected");
	powers.fill(0.0f);
...
void check_displayed_guitar_single_note_probe_profile(Runner &runner)
{
	...
	InstrumentState blank_after_residue = {};
	std::snprintf(blank_after_residue.label, sizeof(blank_after_residue.label), "--");
	ChordResult smoothed_am_source = make_guitar_plain_triad(9, true, 1.00f);
	std::snprintf(smoothed_am_source.label, sizeof(smoothed_am_source.label), "Am=Am7=C6");
	NoteGrid measured_am_display = {};
	set_midi(measured_am_display, 45, 1.00f);
	set_midi(measured_am_display, 57, 1.00f);
	set_midi(measured_am_display, 64, 0.33f);
	NoteGrid measured_am_analysis = {};
	set_midi(measured_am_analysis, 72, 0.09f);
	set_midi(measured_am_analysis, 73, 0.06f);
	set_midi(measured_am_analysis, 86, 0.01f);
	set_midi(measured_am_analysis, 75, 0.03f);
	set_midi(measured_am_analysis, 64, 0.30f);
	set_midi(measured_am_analysis, 56, 0.06f);
	set_midi(measured_am_analysis, 57, 1.00f);
	NoteGrid measured_am_smoothed = {};
	set_midi(measured_am_smoothed, 72, 0.09f);
	set_midi(measured_am_smoothed, 64, 0.30f);
	set_midi(measured_am_smoothed, 57, 1.00f);
	std::array<float, kNoteProbeCount> measured_am_probe = {};
	set_probe_level(measured_am_probe, 45, 1.00f);
	set_probe_level(measured_am_probe, 60, 0.447f);
	set_probe_level(measured_am_probe, 64, 0.407f);
	...
	runner.expect(std::strcmp(blank_after_residue.label, "Am") == 0,
		      std::string("displayed guitar clean smoothed recovery: expected Am label, got `") +
			      blank_after_residue.label + "`");

	InstrumentState blocked_opposite_third = {};
	std::snprintf(blocked_opposite_third.label, sizeof(blocked_opposite_third.label), "--");
	NoteGrid opposite_third_analysis = measured_am_analysis;
	set_midi(opposite_third_analysis, 61, 0.24f);
...
FullMixDebugCandidate make_other_owned_electronic_keyboard_octave_up_debug(int midi)
{
	FullMixDebugCandidate debug = {};
	debug.midi = midi;
	debug.owner = InstrumentKind::Other;
	...
	debug.pitch_confidence = 0.04f;
	debug.periodicity = 0.39f;
	debug.harmonic_fit_error = 4.50f;
	debug.local_noise_level = 0.29f;
	debug.spectral_centroid = 0.65f;
	debug.spectral_slope = 1.46f;
	debug.third_octave_ratio = 1.0f;
	debug.harmonic_ratios[1] = 5.60f;
	debug.harmonic_ratios[2] = 0.34f;
	debug.harmonic_ratios[3] = 9.60f;
...
void check_missing_low_g1_keyboard_octave_alias_mirrors_without_lower_note(Runner &runner)
{
	...
	debug.ownership_confidence = 1.0f;
	debug.guitar_score = 1.0f;
	debug.spectral_level = 0.95f;
	debug.pitch_confidence = 0.75f;
	debug.periodicity = 0.72f;
	debug.harmonic_fit_error = 0.06f;
	debug.local_noise_level = 0.45f;
	debug.spectral_centroid = 0.20f;
	debug.spectral_slope = 0.12f;
	debug.harmonic_ratios = {1.0f, 0.45f, 0.10f, 0.05f, 0.02f};

	runner.expect(full_mix_display_mirror_midi(FullMixDisplayRow::Keyboard, debug, ownership) ==
			      kLowerMidi,
		      "missing low G1 keyboard alias: expected lower fundamental display pitch");
	runner.expect(full_mix_display_mirror_supported(FullMixDisplayRow::Keyboard, debug, kLowerMidi),
		      "missing low G1 keyboard alias: expected measured mirror admission");
	const NoteCandidateList candidates =
		full_mix_display_candidates(ownership, FullMixDisplayRow::Keyboard);
	runner.expect(candidate_list_has_midi(candidates, kLowerMidi),
		      "missing low G1 keyboard alias: expected lower candidate without lower support");
	runner.expect(!candidate_list_has_midi(candidates, kAliasMidi),
		      "missing low G1 keyboard alias: expected octave harmonic to stay hidden");
...
void check_other_owned_electronic_keyboard_alias_mirrors_upper_note(Runner &runner)
{
	...
	ownership.debug_candidate_count = 1;
	ownership.debug_candidates[0] =
		make_other_owned_electronic_keyboard_octave_up_debug(kAliasMidi);

	const NoteCandidateList candidates =
		full_mix_display_candidates(ownership, FullMixDisplayRow::Keyboard);
	runner.expect(candidate_list_has_midi(candidates, kUpperMidi),
		      "other-owned electronic keyboard octave alias: expected upper keyboard note");
	runner.expect(!candidate_list_has_midi(candidates, kAliasMidi),
		      "other-owned electronic keyboard octave alias: expected lower alias not to be mirrored");

	FullMixOwnership weak_shape = ownership;
	weak_shape.debug_candidates[0].harmonic_ratios[3] = 7.80f;
	const NoteCandidateList weak_shape_candidates =
		full_mix_display_candidates(weak_shape, FullMixDisplayRow::Keyboard);
...
void check_measured_low_keyboard_octave_alias_becomes_primary(Runner &runner)
{
	static constexpr int kLowerMidi = 35;
	static constexpr int kAliasMidi = kLowerMidi + 12;
	...
	NoteGrid grid = {};
	write_note_grid_cell(grid, NoteCandidate{kAliasMidi, 1.0f}, 1.0f, 1.0f);
	write_note_grid_cell(grid, NoteCandidate{kLowerMidi, 0.18f}, 1.0f, 1.0f);
	InstrumentState state = {};
	write_note_grid_label(state, grid, -1);
	...
	std::array<float, kNoteProbeCount> powers = {};
	std::array<float, kNoteProbeCount> raw_powers = {};
	set_probe_level(powers, kLowerMidi, 0.012f);
	set_probe_level(powers, kAliasMidi, 0.44f);
	set_probe_level(raw_powers, kLowerMidi, 0.012f);
	set_probe_level(raw_powers, kAliasMidi, 0.44f);
	...
	const NoteCell primary =
		note_grid_primary_cell_for_pitch_class(grid, midi_pitch_class(kLowerMidi));
	runner.expect(primary.active && primary.midi == kLowerMidi,
		      "low keyboard octave alias primary: expected supported lower note to replace alias");
	...
	write_note_grid_label(unsupported_state, unsupported_grid, -1);
	FullMixOwnership unsupported_ownership = ownership;
	unsupported_ownership.global_note_levels[static_cast<std::size_t>(kLowerMidi -
										kFirstMidi)] = 0.0f;
	std::array<float, kNoteProbeCount> unsupported_powers = {};
	std::array<float, kNoteProbeCount> unsupported_raw_powers = {};
	set_probe_level(unsupported_powers, kAliasMidi, 0.44f);
	set_probe_level(unsupported_raw_powers, kAliasMidi, 0.44f);

	prefer_measured_low_keyboard_octave_alias_primary(unsupported_grid, unsupported_state,
							  unsupported_ownership,
							  unsupported_powers,
							  unsupported_raw_powers, -1);
...
void check_probe_supported_guitar_rootless_minor_sixth_alias(Runner &runner)
{
	InstrumentState state = {};
	std::snprintf(state.label, sizeof(state.label), "E=C#m");
	state.confidence = 0.534f;
	NoteGrid display_grid = {};
	set_pitch(display_grid, 4, 0.76f);
	set_pitch(display_grid, 8, 0.70f);
	set_pitch(display_grid, 10, 0.27f);
	NoteGrid analysis_grid = display_grid;
	set_pitch(analysis_grid, 5, 0.01f);
	set_pitch(analysis_grid, 6, 0.03f);
	std::array<float, kNoteProbeCount> powers = {};
	set_probe_level(powers, 49, 0.166f);
	set_probe_level(powers, 52, 1.000f);
	set_probe_level(powers, 53, 0.012f);
	set_probe_level(powers, 56, 0.920f);
	set_probe_level(powers, 58, 0.358f);

	append_probe_supported_guitar_rootless_minor_sixth_alias_after_final_prune(
		state, display_grid, analysis_grid, powers, kGuitarMinMidi, kGuitarMaxMidi);
	runner.expect(chord_label_has_exact_component(state.label, "C#m6"),
		      std::string("rootless minor sixth: expected C#m6 recovered, got `") + state.label +
			      "`");
	runner.expect(chord_label_has_exact_component(state.label, "A#m7b5"),
		      std::string("rootless minor sixth: expected equivalent A#m7b5 recovered, got `") +
			      state.label + "`");

	InstrumentState root_present = {};
	std::snprintf(root_present.label, sizeof(root_present.label), "E=C#m");
	root_present.confidence = 0.534f;
	NoteGrid root_present_analysis = analysis_grid;
	set_pitch(root_present_analysis, 1, 0.06f);
	append_probe_supported_guitar_rootless_minor_sixth_alias_after_final_prune(
		root_present, display_grid, root_present_analysis, powers, kGuitarMinMidi,
		kGuitarMaxMidi);
	runner.expect(!chord_label_has_exact_component(root_present.label, "C#m6"),
		      std::string("rootless minor sixth: expected analysis root to block C#m6, got `") +
			      root_present.label + "`");
}
