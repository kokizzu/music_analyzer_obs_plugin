...
def main() -> int:
    ...
    for target, delegated in {
        ...
        assert 'PATTERN_ARGS="$(MEASURE_GUITAR_ROUTE_PATTERN_ARGS) $(GUITAR_CHORD_ROUTE_PROTECTED_ARGS)' in recipe, (
            f"{target} must use bounded route-report guitar pattern args and cross-corpus protection"
        )
    ...
    assert "DETECTOR_REAL_NOTE_PATTERN_OPTIONAL_CANDIDATE_PATHS += $(IRMAS_ATTRIBUTE_OUTPUT)" in makefile, (
        "detector route scans should mine candidates from independent IRMAS routing rows"
    )
    ...
    assert "DETECTOR_REAL_NOTE_PATTERN_OPTIONAL_PROTECTED_PATHS += $(IRMAS_ATTRIBUTE_OUTPUT)" in makefile, (
        "detector route scans must also protect the independent IRMAS routing rows"
    )
    ...
    for text in [
        "$(BUILD_DIR)/analyzer_guitarset",
        "MUSIC_ANALYZER_GUITARSET_ATTRIBUTE_TSV=\"$@\"",
        "MUSIC_ANALYZER_GUITARSET_MIN_ACTIVE_NOTES=3",
        "MUSIC_ANALYZER_GUITARSET_MIN_PITCH_CLASSES=3",
