...
class DetectionAccuracyReportTest(unittest.TestCase):
    ...
    def test_render_reports_global_and_family_counts(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            piano_chord_bassbonus000 = Path(temporary) / "piano_chord_bassbonus000.txt"
            piano_chord_bassbonus000.write_text(
                "piano_chord_confirmation_audit: baseline_correct=96/530 baseline_wrong=243 "
                "baseline_flickers=0 trial_correct=102/530 trial_wrong=237 trial_flickers=0 "
        ...
        self.assertIn("| 1. Calibrate drum detection | 3 / 3 (100.0%) | 1 / 1 (100.0%) |", report)
        self.assertIn("| 2. Stabilize chord state | 2 / 2 (100.0%) | 1 / 1 (100.0%) |", report)
        self.assertIn("| 4. Safe live Beat This! | 2 / 2 (100.0%) | 1 / 1 (100.0%) |", report)
        ...
        self.assertIn("| Early-onset HiHat | 139→142 / 28→28 | 39→39 / 0→0 | 140→142 / 38→38 |", report)
        self.assertIn("| Zero-false cross-real recovery shapes replayed through runtime gates | 3 / 3 (100.0%) | 0 |", report)
        self.assertIn("| Recovery shapes with a verified overall runtime gain | 1 / 3 (33.3%) | 2 |", report)
        ...
        self.assertIn("| Compile and model-free test the OBS sidecar boundary | 1 / 1 (100.0%) | 0 |", report)
        self.assertIn("| Integrate bounded causal Beat This! live use | 1 / 1 (100.0%) | 0 |", report)
