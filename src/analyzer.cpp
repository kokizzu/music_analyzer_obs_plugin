...
float candidate_score_for_midi(const NoteCandidateList &candidates, int midi)
{
	for (const NoteCandidate &candidate : candidates) {
		if (candidate.midi == midi)
...
bool measured_low_full_mix_vocal_display_supported(const FullMixDebugCandidate &debug)
{
	...
	return debug.spectral_level >= 0.90f &&
	       ...
	       debug.harmonic_fit_error <= 0.18f &&
	       debug.local_noise_level >= 0.24f &&
	       debug.local_noise_level <= 0.90f &&
	       debug.adjacent_upper_ratio >= 0.90f &&
	       second >= 0.10f &&
	       second <= 0.95f &&
	       third <= 0.36f &&
	       fourth <= 0.34f &&
...
bool measured_low_vocal_fundamental_alias_supported(const FullMixDebugCandidate &debug)
{
	...
	const float fourth = debug.harmonic_ratios[3];
	const float fifth = debug.harmonic_ratios[4];
	const bool strong_low_pitch =
		debug.spectral_level >= 0.65f &&
		debug.pitch_confidence >= 0.50f &&
		debug.periodicity >= 0.66f &&
		debug.harmonic_fit_error <= 0.36f;
	const bool formant_like_low_vowel =
		debug.spectral_centroid >= 0.32f &&
		debug.spectral_slope >= 0.55f &&
		second >= 0.30f &&
		third >= 0.10f &&
		(fourth >= 0.08f || fifth >= 0.04f);
	const bool obvious_clean_bass =
		debug.spectral_centroid <= 0.24f &&
		debug.spectral_slope <= 0.32f &&
		third <= 0.12f &&
		fourth <= 0.08f &&
		fifth <= 0.04f;
	return strong_low_pitch && formant_like_low_vowel && !obvious_clean_bass;
}

bool measured_low_vocal_alias_display_midi_supported(const FullMixOwnership &ownership, int display_midi)
{
	if (display_midi < kFullMixVocalMinMidi || display_midi > kVocalMaxMidi)
		return false;

	const int source_midi = display_midi - 12;
	const std::size_t debug_count =
		std::min<std::size_t>(ownership.debug_candidate_count, ownership.debug_candidates.size());
	for (std::size_t i = 0; i < debug_count; ++i) {
		const FullMixDebugCandidate &debug = ownership.debug_candidates[i];
		if (debug.midi == source_midi && measured_low_vocal_fundamental_alias_supported(debug))
			return true;
	}
	return false;
}
...
bool measured_mid_acoustic_other_body_supported(const FullMixDebugCandidate &debug)
{
	if (debug.owner != InstrumentKind::Vocal && debug.owner != InstrumentKind::Guitar)
		return false;
	if (debug.midi < 60 || debug.midi > 65)
		return false;
	...
	const float fourth = debug.harmonic_ratios[3];
	const float fifth = debug.harmonic_ratios[4];
	const bool vocal_owned_body =
		debug.owner == InstrumentKind::Vocal &&
		debug.vocal_score >= 0.54f &&
		debug.vocal_score <= 0.59f &&
		debug.guitar_score >= 0.30f &&
		debug.guitar_score <= 0.38f;
	const bool guitar_owned_body =
		debug.owner == InstrumentKind::Guitar &&
		debug.guitar_score >= 0.62f &&
	...
	return (vocal_owned_body || guitar_owned_body) &&
	       ...
	       debug.harmonic_fit_error <= 0.027f &&
	       debug.harmonicity >= 0.40f &&
	       debug.harmonicity <= 0.43f &&
	       debug.local_noise_level <= 0.080f &&
	       debug.spectral_centroid >= 0.145f &&
	       debug.spectral_centroid <= 0.175f &&
...
bool measured_low_acoustic_string_body_supported(const FullMixDebugCandidate &debug)
{
	...
	const bool vocal_owned_body =
		...
		debug.keyboard_score >= 0.19f &&
		debug.keyboard_score <= 0.205f;
	const bool keyboard_owned_body =
		debug.owner == InstrumentKind::Keyboard &&
		debug.keyboard_score >= 0.99f &&
		debug.vocal_score <= 0.010f;
	return (vocal_owned_body || keyboard_owned_body) &&
	       debug.guitar_score <= 0.010f &&
	       debug.other_score <= 0.010f &&
	       debug.spectral_level >= 0.90f &&
	       debug.pitch_confidence >= 0.82f &&
	       ...
	       debug.periodicity <= 0.72f &&
	       debug.harmonic_fit_error <= 0.040f &&
	       debug.harmonicity <= 0.27f &&
	       debug.local_noise_level >= 0.12f &&
	       debug.local_noise_level <= 0.27f &&
	       debug.spectral_centroid <= 0.11f &&
	       debug.spectral_slope <= 0.09f &&
	       debug.adjacent_lower_ratio >= 0.23f &&
	       debug.third_octave_ratio <= 0.003f &&
	       second >= 0.050f &&
	       second <= 0.20f &&
	       third <= 0.055f &&
	       fourth <= 0.010f &&
				 ...
				 const FullMixOwnership &ownership)
{
	...
	if (row == FullMixDisplayRow::Vocal &&
		...
		const int lowered = debug.midi - 12;
		if (lowered >= kFullMixVocalMinMidi &&
		    ownership_global_note_level(ownership, lowered) >=
			    std::max(0.08f, ownership_global_note_level(ownership, debug.midi) * 0.34f))
			return lowered;
	}
	if (row == FullMixDisplayRow::Vocal && measured_low_vocal_fundamental_alias_supported(debug)) {
		const int raised = debug.midi + 12;
		if (raised >= kFullMixVocalMinMidi && raised <= kVocalMaxMidi)
			return raised;
	}
...
bool keyboard_owned_mid_acoustic_guitar_body_supported(const FullMixDebugCandidate &debug)
{
	return debug.owner == InstrumentKind::Keyboard &&
...
bool vocal_owned_measured_acoustic_piano_body_supported(const FullMixDebugCandidate &debug)
{
	const float second = debug.harmonic_ratios[1];
	const float third = debug.harmonic_ratios[2];
	const float fourth = debug.harmonic_ratios[3];
	...
	const bool resonant_mid_keyboard_body =
		debug.owner == InstrumentKind::Vocal &&
		debug.midi == 58 &&
		debug.vocal_score >= 0.80f &&
		debug.vocal_score <= 0.85f &&
		debug.keyboard_score >= 0.155f &&
		debug.keyboard_score <= 0.205f &&
		debug.guitar_score <= 0.010f &&
		debug.other_score <= 0.010f &&
		debug.spectral_level >= 0.90f &&
		debug.pitch_confidence >= 0.868f &&
		debug.pitch_confidence <= 0.92f &&
		debug.periodicity >= 0.71f &&
		debug.periodicity <= 0.78f &&
		debug.harmonic_fit_error <= 0.055f &&
	...
	const bool high_piano_vocal_body =
		debug.owner == InstrumentKind::Vocal &&
		debug.midi == 84 &&
		debug.vocal_score >= 0.99f &&
		debug.keyboard_score <= 0.001f &&
		debug.guitar_score <= 0.001f &&
		debug.other_score <= 0.001f;
	const bool high_piano_ambiguous_body =
		debug.owner == InstrumentKind::Ambiguous &&
		debug.midi == 84 &&
		debug.ownership_confidence <= 0.001f &&
		debug.keyboard_score <= 0.001f &&
		...
		debug.vocal_score <= 0.001f &&
		debug.other_score <= 0.001f;
	const bool high_mallet_keyboard_body =
		(high_piano_vocal_body || high_piano_ambiguous_body) &&
		debug.spectral_level >= 0.90f &&
		debug.pitch_confidence >= 0.90f &&
		debug.pitch_confidence <= 0.94f &&
		debug.periodicity >= 0.70f &&
		debug.periodicity <= 0.75f &&
		debug.harmonic_fit_error >= 0.045f &&
		debug.harmonic_fit_error <= 0.11f &&
		debug.local_noise_level <= 0.075f &&
		debug.spectral_centroid <= 0.16f &&
	...
	return resonant_mid_keyboard_body || high_mallet_keyboard_body;
}

bool vocal_owned_noisy_low_electronic_keyboard_body_supported(const FullMixDebugCandidate &debug)
{
	if (debug.owner != InstrumentKind::Vocal)
		return false;
	if (debug.midi != 53)
	...
	return debug.vocal_score >= 0.79f &&
	       debug.vocal_score <= 0.805f &&
	       debug.keyboard_score >= 0.196f &&
	       debug.keyboard_score <= 0.205f &&
	       debug.guitar_score <= 0.010f &&
	       debug.other_score <= 0.010f &&
	       debug.spectral_level >= 0.90f &&
	       debug.pitch_confidence >= 0.85f &&
	       debug.pitch_confidence <= 0.878f &&
	       debug.periodicity >= 0.68f &&
	       debug.periodicity <= 0.715f &&
	       debug.harmonic_fit_error >= 0.008f &&
	       debug.harmonic_fit_error <= 0.025f &&
	       debug.harmonicity <= 0.22f &&
	       debug.local_noise_level >= 0.173f &&
	       debug.local_noise_level <= 0.24f &&
	       debug.spectral_centroid <= 0.10f &&
	       debug.spectral_slope <= 0.09f &&
	       debug.adjacent_lower_ratio >= 0.47f &&
	       debug.adjacent_upper_ratio >= 0.42f &&
	       debug.third_octave_ratio <= 0.001f &&
	       second >= 0.10f &&
	       second <= 0.12f &&
	       third >= 0.035f &&
...
bool vocal_owned_guitar_blend_electronic_keyboard_body_supported(const FullMixDebugCandidate &debug)
{
	if (debug.owner != InstrumentKind::Vocal)
		return false;
	if (debug.midi < 55 || debug.midi > 64)
		return false;
	...
	const float fourth = debug.harmonic_ratios[3];
	const float fifth = debug.harmonic_ratios[4];
	return debug.vocal_score >= 0.54f &&
	       debug.vocal_score <= 0.60f &&
	       debug.keyboard_score <= 0.020f &&
	       debug.guitar_score >= 0.39f &&
	       debug.guitar_score <= 0.43f &&
	       debug.other_score <= 0.010f &&
	       debug.spectral_level >= 0.90f &&
	       debug.pitch_confidence >= 0.84f &&
	       debug.periodicity >= 0.82f &&
				       ...
				       int display_midi)
{
	...
	case FullMixDisplayRow::Keyboard: {
		...
		return debug.owner == InstrumentKind::Keyboard ||
		       ...
		       vocal_owned_measured_acoustic_piano_body_supported(debug) ||
		       vocal_owned_noisy_low_electronic_keyboard_body_supported(debug) ||
	...
	case FullMixDisplayRow::Other:
		...
		const bool measured_string_other =
			...
			low_acoustic_string_other_display_supported(debug) ||
			measured_mid_acoustic_other_body_supported(debug) ||
			measured_low_acoustic_string_body_supported(debug) ||
						...
						int min_midi, int preferred_root)
{
	...
	for (int pitch_class = 0; pitch_class < 12; ++pitch_class) {
		...
		for (int octave_delta : {12, 24}) {
			...
			const float lower_second_octave_ratio =
				probe_level(powers, lower_midi + 24) / primary_probe;
			const float lower_major_third_ratio =
				probe_level(powers, lower_midi + 28) / primary_probe;
			const bool low_e1_supported =
				primary.midi == 40 && lower_midi == 28 && lower_ratio >= 0.030f &&
				lower_ratio <= 0.045f && lower_second_octave_ratio >= 0.70f &&
				lower_major_third_ratio >= 0.20f;
			...
			if (!low_e1_supported && !low_g1_supported && !direct_octave_supported &&
			    !double_octave_supported)
				continue;
			if (fundamental_primary_supported && !low_e1_supported && !low_g1_supported)
				continue;
					       ...
					       int preferred_root)
{
	...
	for (int pitch_class = 0; pitch_class < 12; ++pitch_class) {
		...
		for (int octave_delta : {12, 24}) {
			...
			const bool visible_lower_supported = lower_level >= 0.65f;
			const bool probe_supported = visible_lower_supported && lower_ratio >= 0.080f;
			const bool weak_acoustic_e1_supported =
				...
				lower_ratio <= 0.020f;
			const float primary_raw_probe = probe_level(raw_powers, primary.midi);
			const float lower_raw_ratio = primary_raw_probe > 1.0e-6f ?
							  ...
							  const NoteGrid &analysis_grid)
{
	...
	if (component.quality == RootChordQuality::Major || component.quality == RootChordQuality::Minor) {
		...
		const bool extension_component =
			suffix_is(suffix, suffix_len, "6") || suffix_is(suffix, suffix_len, "m6") ||
			...
			suffix_is(suffix, suffix_len, "m9") || suffix_is(suffix, suffix_len, "maj9") ||
			suffix_is(suffix, suffix_len, "add9");
		if (extension_component &&
		    !guitar_candidate_alias_supported_for_display(start, len, display_grid, analysis_grid))
			return false;
		...
		if (suffix_is(suffix, suffix_len, "6") || suffix_is(suffix, suffix_len, "m6")) {
			if (supported_alias_tone(component.root + 9, 0.24f, 0.12f, 0.10f))
				return true;
		} else if (suffix_is(suffix, suffix_len, "7") || suffix_is(suffix, suffix_len, "m7")) {
			if (supported_alias_tone(component.root + 10, 0.24f, 0.12f, 0.10f))
				return true;
		} else if (suffix_is(suffix, suffix_len, "maj7")) {
			if (supported_alias_tone(component.root + 11, 0.24f, 0.12f, 0.10f))
				return true;
		} else if (suffix_is(suffix, suffix_len, "9") || suffix_is(suffix, suffix_len, "m9") ||
			   suffix_is(suffix, suffix_len, "maj9") ||
			   suffix_is(suffix, suffix_len, "add9")) {
			if (supported_alias_tone(component.root + 2, 0.26f, 0.14f, 0.10f))
				return true;
		}
						     ...
						     int min_midi, int max_midi)
{
	...
	for (const RootToScan &entry : roots) {
		...
		const bool omitted_fifth_major_core =
			!fifth_supported && major_supported &&
			major_support >= std::max(0.16f, root_support * 0.50f);
		const bool omitted_fifth_minor_core =
			!fifth_supported && minor_supported &&
			minor_support >= std::max(0.16f, root_support * 0.50f);
		const float major_core_anchor =
			std::min({root_support, major_support,
				  fifth_supported ? fifth_support : major_support});
		const float minor_core_anchor =
			std::min({root_support, minor_support,
				  fifth_supported ? fifth_support : minor_support});
		...
		const float ninth = probe_norm(root + 2);

		if (major_supported && fifth_supported) {
			if (no_third)
				append_chord_alias(chord, root, "");
			if (supported_extension(root, 10, flat_seventh, major_core_anchor) &&
			    supported_extension(root, 2, ninth, major_core_anchor))
				append_chord_alias(chord, root, "9");
			if (supported_extension(root, 11, major_seventh, major_core_anchor) &&
			    supported_extension(root, 2, ninth, major_core_anchor))
				append_chord_alias(chord, root, "maj9");
			if (supported_extension(root, 10, flat_seventh, major_core_anchor))
			...
			if (supported_extension(root, 2, ninth, major_core_anchor))
				append_chord_alias(chord, root, "add9");
		} else if (omitted_fifth_major_core) {
			if (supported_extension(root, 10, flat_seventh, major_core_anchor))
				append_chord_alias(chord, root, "7");
		}
		if (minor_supported && fifth_supported) {
			if (no_third)
				append_chord_alias(chord, root, "m");
			if (supported_extension(root, 10, flat_seventh, minor_core_anchor) &&
			    supported_extension(root, 2, ninth, minor_core_anchor))
				append_chord_alias(chord, root, "m9");
			if (supported_extension(root, 10, flat_seventh, minor_core_anchor))
				append_chord_alias(chord, root, "m7");
		} else if (omitted_fifth_minor_core) {
			if (supported_extension(root, 10, flat_seventh, minor_core_anchor))
				append_chord_alias(chord, root, "m7");
		}
	}
...
void append_analysis_complete_guitar_extension_aliases(ChordResult &chord, const NoteGrid &display_grid,
						       const NoteGrid &analysis_grid)
{
	if (chord.root < 0 || !chord.label[0] || chord.label[0] == '-')
		return;
	...
	for (int root = 0; root < 12; ++root) {
		const bool root_labeled = chord_label_has_root_component(chord.label, root);
		for (const ExtensionTemplate &chord_template : kTemplates) {
			...
			for (int i = 0; i < chord_template.count; ++i) {
				const int interval = chord_template.intervals[static_cast<std::size_t>(i)];
				const int pitch_class = root + interval;
				if (note_grid_pitch_active(analysis_grid, pitch_class))
					++analysis_tones;
				if (note_grid_pitch_active(display_grid, pitch_class))
					++display_tones;
				if (interval == 0 || interval == 3 || interval == 4 || interval == 6 || interval == 7) {
					weakest_core =
						std::min(weakest_core,
							 note_grid_pitch_level(analysis_grid, pitch_class));
				}
			}
			if (analysis_tones < chord_template.count || display_tones < 2)
				continue;
			if (!root_labeled && display_tones < 3 && weakest_core < std::max(0.055f, strongest_analysis * 0.035f))
				continue;
			append_chord_alias(chord, root, chord_template.suffix);
		}
	}
}
					 ...
					 const char *source_name, uint64_t dropped_windows)
{
	...
	if (one_shot_measured_low_body_ride_hihat_steal) {
		...
		const bool one_shot_measured_inactive_tom_from_kick_primary_recovery =
			...
			((drum_level_[Kick] <= 0.987f &&
			  tom_kick_shape_score_ratio <= 0.482f &&
			  tom_kick_body_ratio <= 1.127f) ||
			 (drum_level_[Kick] <= 0.989f &&
			  snare_kick_shape_score_ratio <= 0.178f));
		...
		const bool one_shot_measured_tom_kick_low_crash_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			snapshot.drum_debug_trigger_scores[Crash] <= 61.418f &&
			snapshot.low_energy >= 0.64f &&
			snare_crack >= 17.458f;
		...
		const bool one_shot_measured_mid_tom_from_snare_primary_recovery =
			drum_detection_enabled &&
			body_shape == Tom &&
			drum_level_[Snare] > 0.30f &&
			snapshot.high_energy <= 0.25f &&
			snapshot.mid_energy >= 0.668f &&
			tom_kick_band_ratio <= 1.951f;
		...
		const bool one_shot_measured_kick_band_tom_from_snare_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			measured_one_shot_tom_shape &&
			drum_bands[Kick] >= 134.45f &&
			drum_level_[Kick] <= 0.001f &&
			drum_segment_bands[Kick] <= 89.03f;
		const bool one_shot_measured_short_body_tom_from_snare_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			body_shape == Tom &&
			tom_body <= 52.81f &&
			body_shape_scores[2] >= 60.72f;
		const bool one_shot_measured_low_segment_tom_from_snare_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			body_shape == Tom &&
			drum_segment_bands[Tom] <= 59.49f &&
			body_shape_scores[2] >= 60.72f &&
			upper_tom_body >= 28.01f;
		...
		const bool one_shot_measured_rim_trigger_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Snare] > 0.30f &&
			drum_level_[Tom] > 0.30f &&
			snapshot.drum_debug_trigger_scores[Rim] >= 25.601f &&
			snapshot.drum_debug_trigger_scores[Snare] <= 24.342f;
		...
		const float measured_tom_kick_level_ratio =
			drum_level_[Tom] / (drum_level_[Kick] + 1.0e-9f);
		const bool one_shot_measured_saturated_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Snare] > 0.30f &&
			measured_hihat_rim_level_ratio >= 926085000.0f &&
			measured_tom_kick_level_ratio >= 999999900.0f;
		const bool one_shot_measured_high_band_snare_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Snare] <= 0.30f &&
			drum_level_[Kick] <= 0.30f &&
			drum_level_[Tom] <= 0.30f &&
			drum_level_[Rim] <= 0.30f &&
			drum_bands[HiHat] >= 38.772f &&
			tom_snare_total_band_ratio >= 4.20f &&
			tom_snare_trigger_ratio <= 0.955f &&
			snapshot.drum_debug_trigger_scores[Snare] >= 35.0f &&
			snare_crack >= 4.50f;
		const bool one_shot_measured_bright_segment_snare_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Snare] <= 0.001f &&
			drum_level_[Kick] <= 0.30f &&
			drum_level_[Tom] <= 0.30f &&
			drum_level_[Rim] <= 0.30f &&
			drum_segment_bands[Crash] >= 21.692f &&
			drum_segment_bands[Snare] >= 17.166f &&
			snapshot.drum_debug_trigger_scores[Snare] <= 63.596f;
		const bool one_shot_measured_low_cymbal_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Snare] > 0.30f &&
			drum_level_[Tom] > drum_level_[Snare] &&
			snapshot.low_energy <= 0.30f &&
			snapshot.mid_energy >= 0.50f &&
			snapshot.high_energy <= 0.20f &&
			hihat_rim_band_ratio >= 0.56f &&
			hihat_rim_shape_score_ratio <= 0.11f &&
			snare_body >= 70.0f &&
			snare_crack >= snare_body * 0.12f &&
			snare_crack <= snare_body * 0.18f &&
			tom_body <= snare_body * 1.80f &&
			snapshot.drum_debug_trigger_scores[Snare] >=
				snapshot.drum_debug_trigger_scores[Tom] * 1.24f;
		const bool one_shot_measured_triggered_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			body_shape == Tom &&
			drum_level_[Snare] > 0.30f &&
			drum_level_[Tom] > 0.30f &&
			snare_kick_shape_score_ratio >= 2.178f &&
			snapshot.drum_debug_trigger_thresholds[Tom] <= 0.426f &&
			snapshot.drum_debug_trigger_scores[Tom] >= 27.015f;
		const bool one_shot_measured_mid_snare_ambiguous_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Snare] > 0.30f &&
			snapshot.mid_energy >= 0.702f &&
			snare_kick_level_ratio >= 3.50f &&
			snare_kick_trigger_ratio <= 1.207f;
		const bool one_shot_measured_bright_rim_snare_ambiguous_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			body_shape == Snare &&
			drum_level_[Snare] > 0.30f &&
			hihat_rim_level_ratio >= 1.045f &&
			drum_bands[Rim] >= 47.258f;
		...
		const bool one_shot_measured_low_body_crash_from_rim_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Crash] > 0.30f &&
			drum_level_[Rim] > 0.30f &&
			hihat_rim_level_ratio >= 0.899f &&
			hihat_rim_level_ratio <= 0.962f &&
			snare_body <= 17.152f &&
			snare_crack >= 8.684f;
		const bool one_shot_measured_hihat_weighted_crash_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Crash] > 0.30f &&
			drum_level_[Crash] <= 0.703f &&
			crash_hihat_level_ratio >= 0.349f &&
			hihat_rim_trigger_ratio <= 0.941f;
		...
		const bool one_shot_measured_thin_hihat_from_rim_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[HiHat] > 0.30f &&
			drum_level_[Rim] > drum_level_[HiHat] &&
			drum_bands[Crash] >= 0.489f &&
			hihat_rim_level_ratio <= 0.984f &&
			rim_shape_score <= 3.391f;
		...
		const bool one_shot_measured_kick_snare_primary_recovery =
			...
			((drum_bands[Ride] <= 1.67f &&
			 ...
			 (body_shape == Tom &&
			  kick_body >= 193.22f));
		const bool one_shot_measured_mid_body_kick_from_snare_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			hihat_rim_segment_ratio <= 0.265f &&
			snare_kick_level_ratio >= 1.02f &&
			tom_body >= 40.186f &&
			drum_segment_bands[Tom] <= 45.504f;
		...
		if (one_shot_measured_tom_rim_primary_recovery ||
		    ...
		    one_shot_measured_tom_kick_low_crash_primary_recovery ||
		    one_shot_measured_tom_from_snare_primary_recovery ||
		    one_shot_measured_mid_tom_from_snare_primary_recovery ||
		    one_shot_measured_high_ride_tom_snare_primary_recovery ||
		    one_shot_measured_kick_band_tom_from_snare_primary_recovery ||
		    one_shot_measured_short_body_tom_from_snare_primary_recovery ||
		    one_shot_measured_low_segment_tom_from_snare_primary_recovery)
		...
		if (one_shot_measured_hihat_ambiguous_snare_shape_primary_recovery ||
		    ...
		    one_shot_measured_thin_hihat_from_rim_primary_recovery ||
		...
		if (one_shot_measured_crash_from_hihat_band_primary_recovery ||
		    ...
		    one_shot_measured_low_body_crash_from_rim_primary_recovery ||
		    one_shot_measured_hihat_weighted_crash_primary_recovery)
			promote_drum_primary(Crash, 0.90f);
		if (one_shot_measured_snare_rim_primary_recovery ||
		    ...
		    one_shot_measured_low_cymbal_snare_from_tom_primary_recovery ||
		    one_shot_measured_triggered_snare_from_tom_primary_recovery ||
		    one_shot_measured_mid_snare_ambiguous_primary_recovery ||
		    one_shot_measured_bright_rim_snare_ambiguous_primary_recovery)
		...
		if (one_shot_measured_kick_tom_primary_recovery ||
		    ...
		    one_shot_measured_mid_body_kick_from_snare_primary_recovery ||
		...
		const bool one_shot_measured_body_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			body_shape == Snare &&
			drum_level_[Snare] > 0.30f &&
			drum_level_[Tom] > 0.30f &&
			tom_snare_level_ratio >= 1.005f &&
			tom_snare_trigger_ratio <= 0.788f;
		if (one_shot_measured_late_snare_tom_level_primary_recovery ||
		    one_shot_measured_body_snare_from_tom_primary_recovery)
			promote_drum_primary(Snare, 0.90f);
		...
		if (one_shot_measured_narrow_band_tom_from_kick_primary_recovery ||
			...
			promote_drum_primary(Tom, 0.90f);
		const float final_no_kick_snare_tom_level_ratio =
			drum_level_[Tom] / (drum_level_[Snare] + 1.0e-6f);
		const bool one_shot_measured_no_kick_bright_tail_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Snare] > 0.30f &&
			drum_level_[Tom] > 0.30f &&
			drum_level_[Kick] <= 0.001f &&
			final_no_kick_snare_tom_level_ratio >= 1.02f &&
			drum_segment_bands[Crash] >= 7.856f &&
			upper_tom_body >= 69.776f;
		if (one_shot_measured_no_kick_bright_tail_snare_from_tom_primary_recovery)
			promote_drum_primary(Snare, 0.90f);
		...
		const bool one_shot_measured_kick_band_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Tom] > 0.30f &&
			drum_level_[Snare] <= drum_level_[Tom] &&
			drum_shape_supported[Crash] &&
			drum_bands[Kick] >= 197.284f;
		const bool one_shot_measured_ratio_snare_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			drum_level_[Tom] > 0.30f &&
			drum_level_[Snare] <= drum_level_[Tom] &&
		...
		const float late_snare_kick_level_ratio =
			drum_level_[Snare] / (drum_level_[Kick] + 1.0e-6f);
		const bool one_shot_measured_massive_tom_body_snare_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			body_shape == Tom &&
			drum_level_[Kick] > 0.30f &&
			drum_level_[Tom] >= 0.95f &&
			drum_level_[Snare] > drum_level_[Tom] &&
			drum_level_[Snare] <= drum_level_[Tom] + 0.06f &&
			drum_bands[Crash] <= 0.139f &&
			late_snare_kick_level_ratio >= 1.008f &&
			tom_body >= 276.353f;
		if (one_shot_measured_massive_tom_body_snare_primary_recovery) {
			snapshot.drum_debug_rule_flags |= DrumDebugMassiveTomBodySnarePrimaryRecovery;
			promote_drum_primary(Tom, 0.90f);
		}
		...
		if (generated_gm_orchestra_tom_primary_recovery)
			promote_drum_primary(Tom, 0.90f);
		const bool generated_gm_high_tom_primary_recovery =
			drum_detection_enabled && generated_gm_drum_source &&
			drum_level_[Tom] > 0.30f &&
			drum_shape_supported[Tom] &&
			body_shape == Kick &&
			tom_body >= kick_body * 1.60f &&
			tom_body >= snare_body * 1.90f &&
			upper_tom_body >= snare_crack * 6.0f &&
			drum_segment_bands[Tom] >= drum_segment_bands[Kick] * 1.55f &&
			snapshot.mid_energy >= 0.24f &&
			snapshot.high_energy >= 0.08f;
		if (generated_gm_high_tom_primary_recovery)
			snapshot.drum_debug_rule_flags |= DrumDebugGmHighTomRecovery;
		if (generated_gm_high_tom_primary_recovery)
			promote_drum_primary(Tom, 0.90f);
		const bool generated_gm_low_tom_primary_recovery =
			drum_detection_enabled && generated_gm_drum_source &&
			drum_level_[Tom] > 0.30f &&
			drum_shape_supported[Tom] &&
			body_shape == Kick &&
			tom_body >= kick_body * 1.45f &&
			tom_body >= snare_body * 2.40f &&
			upper_tom_body >= snare_crack * 6.0f &&
			drum_segment_bands[Tom] >= drum_segment_bands[Kick] * 1.35f &&
			snapshot.low_energy >= 0.75f &&
		...
		const bool one_shot_measured_tom_snare_crack_bleed_recovery =
			...
			kick_body >= 43.30f &&
			tom_snare_body_ratio <= 1.513f &&
			tom_body >= snare_body * 1.10f &&
			upper_tom_body >= snare_crack * 2.10f;
		const bool measured_snare_crack_tom_bleed =
			...
			snare_crack >= snare_body * 0.18f &&
			tom_body <= snare_body * 1.70f &&
			!one_shot_measured_tom_snare_crack_bleed_recovery &&
			!one_shot_measured_mid_tom_from_snare_primary_recovery &&
		...
		const float final_post_tom_bleed_kick_level_ratio =
			drum_level_[Tom] / (drum_level_[Kick] + 1.0e-6f);
		const bool one_shot_measured_late_deep_kick_from_tom_primary_recovery =
			drum_detection_enabled && one_shot_drum_source &&
			!generated_gm_orchestra_tom_primary_recovery &&
	...
	if (input_mode == AnalysisInputMode::FullMix || input_mode == AnalysisInputMode::IsolatedBass) {
		...
		if (mixed_bass_supported) {
			...
			if (bass_shadowed_by_keyboard_alias &&
				...
				tracked_bass_confidence_ = 0.0f;
				tracked_bass_score_ = 0.0f;
				snapshot.bass_debug_displayed_midi = -1;
				snapshot.bass_debug_displayed_confidence = 0.0f;
				snapshot.bass_debug_displayed_score = 0.0f;
				clear_note_grid(snapshot.bass_notes);
				copy_text(snapshot.bass.label, sizeof(snapshot.bass.label), "--");
				snapshot.bass.confidence = 0.0f;
			} else if (!isolated_bass && tracked_bass_midi_ >= 0 && tracked_bass_misses_ < 2) {
	...
	auto process_guitar = [&]() {
		...
		if (!mixed_source) {
			ChordResult supported_root_third_dyad =
				detect_supported_guitar_root_third_dyad(
					snapshot.guitar_notes, guitar_chord_detection_grid,
					preferred_root);
			if (!valid_chord_result(supported_root_third_dyad) && raw_guitar_valid)
				supported_root_third_dyad =
					detect_compact_guitar_root_third_dyad(
						snapshot.guitar_notes, guitar_chord_detection_grid,
						preferred_root);
			if (valid_chord_result(supported_root_third_dyad)) {
				if (!raw_guitar_valid ||
				    guitar_root_third_dyad_should_replace_weak_analysis_root(
					    raw_guitar_chord, supported_root_third_dyad,
					    snapshot.guitar_notes, guitar_chord_detection_grid)) {
					raw_guitar_chord = supported_root_third_dyad;
					raw_guitar_valid = true;
				} else if (raw_guitar_chord.root == supported_root_third_dyad.root) {
					append_preferred_guitar_root_third_dyad_alias(
						raw_guitar_chord, supported_root_third_dyad);
					raw_guitar_valid = guitar_chord_valid_for_display(raw_guitar_chord);
				}
			}
		...
		if (!mixed_source)
			promote_strong_same_root_guitar_extension_primary(
				...
				note_powers, min_midi, kGuitarMaxMidi);
		if (!mixed_source)
			prune_unsupported_same_root_guitar_extension_aliases(
				raw_guitar_chord, snapshot.guitar_notes, guitar_chord_detection_grid,
				&note_powers, min_midi, kGuitarMaxMidi);
		if (!mixed_source &&
	...
	if (guitar_enabled) {
		...
		if (!mixed_source) {
			ChordResult smoothed_root_third_dyad =
				detect_supported_guitar_root_third_dyad(
					snapshot.guitar_notes, guitar_chord_grid,
					lowest_note_grid_pitch_class(guitar_chord_grid));
			if (!valid_chord_result(smoothed_root_third_dyad) &&
			    valid_chord_result(smoothed_guitar_chord))
				smoothed_root_third_dyad =
					detect_compact_guitar_root_third_dyad(
						snapshot.guitar_notes, guitar_chord_grid,
						lowest_note_grid_pitch_class(guitar_chord_grid));
			if (valid_chord_result(smoothed_root_third_dyad)) {
				if (!valid_chord_result(smoothed_guitar_chord) ||
				    guitar_root_third_dyad_should_replace_weak_analysis_root(
					    smoothed_guitar_chord, smoothed_root_third_dyad,
					    snapshot.guitar_notes, guitar_chord_grid)) {
					smoothed_guitar_chord = smoothed_root_third_dyad;
				} else if (smoothed_guitar_chord.root == smoothed_root_third_dyad.root) {
					append_preferred_guitar_root_third_dyad_alias(
						smoothed_guitar_chord, smoothed_root_third_dyad);
				}
			}
		...
		if (allow_smoothed_extensions) {
			...
			if (!mixed_source)
				prepend_analysis_complete_smoothed_guitar_plain_alias(
					smoothed_guitar_chord, smoothed_guitar_chord,
					snapshot.guitar_notes, guitar_chord_grid);
			if (!mixed_source)
				append_hidden_root_guitar_plain_aliases(
					smoothed_guitar_chord, snapshot.guitar_notes,
					guitar_chord_detection_grid);
			if (!mixed_source)
				prepend_analysis_complete_smoothed_guitar_plain_alias(
		...
		const bool raw_changed_by_smoothed =
			...
			promote_smoothed_same_root_guitar_quality(raw_guitar_chord, smoothed_guitar_chord,
								  note_powers, kGuitarMinMidi,
								  kGuitarMaxMidi);
		const bool raw_promoted_by_visible_smoothed =
			!mixed_source &&
			promote_visible_root_third_component_primary(
				raw_guitar_chord, smoothed_guitar_chord, snapshot.guitar_notes,
				guitar_chord_detection_grid);
		if (!mixed_source)
			prune_unsupported_same_root_guitar_extension_aliases(
		...
		if (!mixed_source) {
			append_compact_guitar_power_raw_profile_aliases_to_chord(
				raw_guitar_chord, snapshot.guitar_notes, guitar_chord_detection_grid,
				note_powers, kGuitarMinMidi, kGuitarMaxMidi);
			append_compact_guitar_power_raw_profile_aliases_to_chord(
				smoothed_guitar_chord, snapshot.guitar_notes, guitar_chord_grid,
		...
		if (raw_changed_by_smoothed || raw_promoted_by_visible_smoothed)
			set_instrument_chord(snapshot.guitar_raw_chord, raw_guitar_chord, guitar_energy, rms,
		...
		if (!mixed_source) {
			...
			append_supported_guitar_display_extension_aliases(
				...
				guitar_chord_detection_grid);
			append_analysis_visible_guitar_display_extension_aliases(
				snapshot.guitar_chord, snapshot.guitar_notes,
				guitar_chord_detection_grid);
			promote_displayed_smoothed_power_guitar_primary(
				snapshot.guitar_chord, smoothed_guitar_chord,
				snapshot.guitar_notes, guitar_chord_detection_grid);
			prune_clean_primary_guitar_display_aliases(
				snapshot.guitar_chord, snapshot.guitar_notes,
				guitar_chord_detection_grid, &note_powers, kGuitarMinMidi,
				kGuitarMaxMidi);
		}
		...
		if (!mixed_source)
			promote_displayed_same_root_plain_guitar_primary(
				...
				kGuitarMinMidi, kGuitarMaxMidi);
		if (!mixed_source)
			promote_displayed_supported_plain_guitar_primary(
				snapshot.guitar_chord, snapshot.guitar_notes,
	...
	if (mixed_source) {
		...
		suppress_vocal_owned_same_pitch_non_vocal_shadows(snapshot.guitar_notes, snapshot.guitar,
								  snapshot.vocal_notes, full_mix_ownership,
								  InstrumentKind::Guitar, -1);
		suppress_vocal_owned_same_pitch_non_vocal_shadows(snapshot.other_notes, snapshot.other,
								  snapshot.vocal_notes, full_mix_ownership,
