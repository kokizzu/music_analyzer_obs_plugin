...
`make analyze-medley-solos-attributes` writes `build/medley_solos_attributes.tsv` after the real Medley-solos archive is available, then prints expected-family x/total recall plus the most frequent wrong-row routes. Each TSV row records the expected family, source instrument, analysis-buffer position, detected family rows, row confidences, and rendered labels. Use it to mine independent sax, violin, woodwind, piano, guitar, and vocal ownership evidence; it does not play audio.
