...
The app sends a complete 8x8 LED refresh after connection and after every effective-root/mode change. The rainbow rotates with the effective root: the root is red, followed chromatically by red-orange, orange, gold, yellow, green, turquoise, light blue, periwinkle, purple, violet, and pink. For example, G is red when the root is G, while C is red when the root is C. The bottom controls use Akai's native dark and light greys. In Auto mode, the dark root/sharp glyph is contrasted by a light-grey center toggle and dark-grey previous/next controls. In Manual mode, the white root/sharp glyph is contrasted by a dark-grey center toggle and light-grey previous/next controls. The implementation uses APC notes 0-63 and Akai's full-brightness solid Note On status `0x96`.
