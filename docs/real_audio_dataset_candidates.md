...
- Use `make test-bach10-mf0-synth-samples` to download the 127 MB
  ...
  bassoon, alto saxophone, clarinet, and violin F0 truth. The current cached
  target checks 40 dense windows, passes with 149/160 pitch-class hits and
  22/40 simplified chord hits, and treats exact extended-chord matching as a
  nonblocking diagnostic because the expected four-note chorale labels often
  collapse to simpler root chords in the detector. Because the audio is
  synthesized/resynthesized, it stays an additional detector stress gate rather
  than a replacement for real recorded URMP or MusicNet coverage. `make
