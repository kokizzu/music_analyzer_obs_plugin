...
| --- | ---: | --- | --- |
| [URMP](https://labsites.rochester.edu/air/projects/URMP.html) | 44 pieces | Real classical duets, trios, quartets, and quintets. Each piece has isolated instrument audio, assembled mixture, MIDI score, and frame/note annotations. The public [Kaggle mirror](https://www.kaggle.com/datasets/alonhaviv/multi-modal-music-performance-urmp/data) is fetched with `make download-real-urmp`, then unpacked non-destructively with `make prepare-real-urmp`; both archive and corpus stay in external `InstrumentSamples/urmp`. | Large download, about 12.5 GB. Classical chamber instruments, not pop rhythm-section stems. |
...
| [Bach10-mf0-synth](https://zenodo.org/records/1481156) | Compact four-part F0-derived note/chord stress gate with `make test-bach10-mf0-synth-samples` | 10 Bach chorales with synthesized/resynthesized mix audio, per-stem audio, and per-stem F0 annotations for bassoon, alto saxophone, clarinet, and violin. The implemented sharded target converts `audio_mix` and `annotation_stems` into a MusicNet-shaped mixed-audio analyzer fixture, enforces pitch recall/precision plus simplified chord recall, and leaves exact 7th/6th/maj7/dim7 extension matching visible as diagnostics. It is useful for extra polyphonic note/chord pressure, but it remains synthesized audio and does not replace URMP or real MusicNet. |
| [STAR Drums](https://zenodo.org/records/15690078) | Mixed-song drum stress gate with `make test-star-drums-samples` | 2025 Separate-Tracks-Annotate-Resynthesize Drums dataset with 18 drum classes, annotations, stems, MIDI, and mixed-song audio. The implemented target uses the 96.7 MB preview ZIP, maps supported drum classes to GM MIDI, and runs the shared drum analyzer gate. The non-drum content is real melodic/vocal audio, but the annotated drums are re-synthesized from automatic transcription, so treat it as diagnostic mixed-song stress rather than real-drum ground truth. |
...
| [MAPS](https://zenodo.org/records/18160555) | Keyboard row, isolated-note, chord, and piano-piece tests with `make test-maps-piano-samples` and `make test-maps-piano-note-samples` | MIDI Aligned Piano Sounds contains MIDI-annotated piano recordings, isolated notes, random-pitch chords, usual musical chords, and pieces in varied recording conditions. The implemented targets download the real Disklavier `ENSTDkCl.zip`, prepare bounded MAESTRO-shaped subsets from isolated-note, usual-chord, random-chord, and music WAV/MIDI pairs, and run the isolated-keyboard analyzer gate. Single instrument only. |
...
| [Real Saxophone Recordings for Audio-to-Score Music Transcription](https://grfia.dlsi.ua.es/audio-to-score/) | Real alto/tenor sax audio with matched **kern scores; tenor probe via `make measure-real-a2s-tenor-scale-probes` | 513 tenor recordings by one professional performer, each paired with a score and technique/tempo metadata. The implemented resumable 3.4-GB archive target stores the source under external `InstrumentSamples/real_a2s_sax`; nine major scales (B♭ through G) plus Exercises 1-3 cut 378 silent score-timed notes, correct the measured one-octave notation offset, and report routing plus exact-MIDI coverage. It is an independent tenor diagnostic, not a broad gate until more recordings and performers are prepared. |
| [University of Iowa Musical Instrument Samples](https://theremin.music.uiowa.edu/MIS.html) | Real single-note acoustic instrument coverage with `make test-iowa-piano-samples`, `make test-iowa-bass-samples`, `make test-iowa-strings-samples`, `make test-iowa-orchestra-samples`, and `make test-iowa-orchestra-full-samples` | Public research/education sample library with pitch-labeled piano AIFF files and post-2012 ZIP pages for woodwinds, brass, strings, and pitched percussion. The implemented full-page target caches page HTML, downloads a bounded one-ZIP-per-page sweep, pitch-checks converted AIF members, and runs the shared real-note analyzer gate. Single-instrument one-note audio only. |
| [OrchideaSOL](https://zenodo.org/records/3740399) | Large isolated-note orchestral sample coverage, especially extended techniques | 13,265 single-note samples across 14 instruments including violin, trombone, bassoon, oboe, tuba, and strings, with MIDI-pitch/technique metadata on Zenodo. Zenodo metadata is directly downloadable; the roughly 1.4 GB audio archive requires free Ircam Forum registration under the Ircam Forum License. It is therefore a strong future independent fixture for violin/trombone/bassoon, but not an unattended downloader and does not add cor anglais/English-horn coverage. |
...
| [ENST-Drums](https://perso.telecom-paristech.fr/grichard/ENST-drums/) | Candidate real-drum Tom/Ride/Rim gate; licence-guarded `make download-enst-drums` acquisition | Three professional drummers, eight recorded microphone channels, stereo drum mixes, accompaniment material, and time/event labels covering rim/cross-stick, multiple toms, ride, and crash. The official public subset requires accepting research-use licence terms. A 2.85 GB 16 kHz prepared [Zenodo archive](https://zenodo.org/records/7831843) is publicly mirrored with MD5 `7e28c2a923e4f4162b3d83877cedb5eb`; the target writes only to external `InstrumentSamples` and requires `ENST_DRUMS_LICENSE_ACCEPTED=1` before it can transfer the archive. |
...
| [MIR-1K](https://zenodo.org/records/3532216) | Independent vocal-with-accompaniment pitch and routing gate | 1,000 short real karaoke clips with accompaniment and vocal channels plus manual frame-level vocal pitch and voicing annotations. `make download-mir1k-dataset`, `make inspect-mir1k-dataset-archive`, `make extract-mir1k-dataset`, `make prepare-mir1k-vocal-mix-samples`, and `make measure-mir1k-vocal-mix` keep the checksum-pinned YourMT3 16 kHz distribution and all generated probe WAVs exclusively in external `InstrumentSamples`; the latter measures stable manually labelled mixture windows, never the isolated vocal stem. |
...
- Use `make test-mdb-drums-samples` to download MDB Drums from GitHub, map
  ...
  139/192 drum-category hits, 69.85% precision, 72.40% recall, and 53.26%
...
- Use `make test-star-drums-samples` to download the 96.7 MB STAR Drums
  ...
  and checks 16 windows, with 39/56 drum-category hits, 100.00% precision,
  69.64% recall, and 0.00% false-positive windows. STAR Drums is useful
...
- Use `make test-bach10-mf0-synth-samples` to download the 127 MB
  ...
  `build/bach10_mf0_synth_musicnet`, and run the mixed-audio MusicNet analyzer
  harness. This adds compact four-part polyphonic note/chord pressure from
  bassoon, alto saxophone, clarinet, and violin F0 truth. The default target
  shards the fixture recordings and aggregates the original pitch, precision,
  and simplified-chord gates; use `make test-bach10-mf0-synth-samples-serial`
  for single-process comparison. The current cached target checks 40 dense
  windows, passes with 143/160 pitch-class hits and 25/40 simplified chord
  hits, and treats exact extended-chord matching as a nonblocking diagnostic
  because the expected four-note chorale labels often collapse to simpler root
  chords in the detector. Because the audio is synthesized/resynthesized, it
  stays an additional detector stress gate rather than a replacement for real
  recorded URMP or MusicNet coverage. `make test-real-world-samples-full` runs
  it only when the archive is already cached.
