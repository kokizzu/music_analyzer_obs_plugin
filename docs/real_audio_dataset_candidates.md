...
| [Real Saxophone Recordings for Audio-to-Score Music Transcription](https://grfia.dlsi.ua.es/audio-to-score/) | Real alto/tenor sax audio with matched **kern scores; initial tenor scale probe via `make measure-real-a2s-tenor-scale-probes` | 513 tenor recordings by one professional performer, each paired with a score and technique/tempo metadata. The implemented resumable 3.4-GB archive target stores the source under external `InstrumentSamples/real_a2s_sax`; the initial G-major-scale probe cuts silent score-timed notes, corrects the measured one-octave notation offset, and reports routing plus exact-MIDI coverage. It is a carefully scoped independent tenor diagnostic, not a broad gate until more recordings are prepared. |
