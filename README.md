...
`make test` also runs `make test-real-goal-fixture`, which unpacks the committed URMP-shaped 20-piece lossless FLAC fixture from `tests/fixtures/urmp-mini.tar.gz`, decodes it to disposable WAV files under `build/` with `ffmpeg`, generates 20-recording MusicNet-shaped plus MedleyDB-shaped, MUSDB18-shaped, Slakh2100-shaped, audio-backed MulTTiPop-shaped, Spheres-shaped, GuitarSet-shaped, MAESTRO-shaped, and E-GMD-shaped fixtures, sends all configured roots through the combined setup preflight, and then sends them through the combined goal gate. That exercises WAV, Notes, MIDI score, full-mix, summed-stem, real-mix-label, MedleyDB/MUSDB18/Spheres stem-layout, Slakh2100 rendered-stem/MIDI layout, MulTTiPop multitrack-MIDI audio analysis, GuitarSet JAMS/hex-audio preflight, MAESTRO MIDI/WAV piano analysis, E-GMD MIDI/WAV drum analysis, and chord paths without downloading external datasets. Override the decoder with `FFMPEG=/path/to/ffmpeg` if needed.
...
`make inspect-real-slakh` is an optional synthesized multitrack truth preflight for Slakh2100. It requires at least 20 complete same-song rendered tracks with mix audio, 4+ stem audio files, readable MIDI, and metadata containing piano, bass, guitar, and drum classes by default. Slakh2100 is useful for large same-song stem/MIDI note and chord coverage, but it does not replace URMP because its audio is MIDI-rendered with virtual instruments rather than real recorded performances.
