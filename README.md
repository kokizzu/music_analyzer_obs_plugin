...
Current local results are 4,371/5,451 pitch-class hits, 66.67% guitar precision, 80.19% guitar recall, 72.81% guitar F1, and 947/1,491 exact chord hits: 590/752 major/minor opportunities and 357/739 other-chord opportunities.
The same run reports 1,049/1,491 simplified-compatible chord hits, including 599/752 major/minor opportunities; this distinguishes missed extensions from completely wrong chord roots. The current miss-analysis report has 544 remaining chord misses, with same-root major/minor opportunities collapsed to power chords reduced to 33.
...
The quality breakdown currently shows major 412/500 exact and 418/500 simplified, minor 177/255 exact and 182/255 simplified, sus2 49/88, sus4 39/88, 7th 84/160 exact and 95/160 simplified, maj7 72/202 exact and 111/202 simplified, m7 73/173 exact and 83/173 simplified, 9th 14/38 exact and 18/38 simplified, add9 26/61 exact and 35/61 simplified, 6th 57/173 exact and 97/173 simplified, m6 18/57 exact and 31/57 simplified, dim 12/26, m7b5 18/57, power 2/2, maj9 1/6 exact and 3/6 simplified, aug 1/27, dim7 0/8, and augmented/diminished-seventh shapes still weak.
...
`make test-guitar-chord-mix-samples` downloads a bounded, label-spread subset of the public [Guitar Chord Mix](https://huggingface.co/datasets/ryangowe/guitar-chord-mix) WAV/JAMS clips into `build/guitar_chord_mix_samples`, writes a GuitarSet-shaped manifest from the per-string `note_midi` annotations, and runs the isolated-guitar note/chord harness. The default `GUITAR_CHORD_MIX_LIMIT=120` keeps the first run moderate; set `GUITAR_CHORD_MIX_LIMIT=0` to prepare all matched WAV/JAMS pairs. The current cached default gate prepares 120 clips with 618 note annotations across 16 chord labels and passes with 81.18% guitar recall, 71.39% guitar precision, 64.52% exact chord recall, and 74.77% exact chord precision. This is real guitar chord audio coverage and is included in `make test-real-world-samples-full`.

`make test-gaps-guitar-samples` downloads a bounded subset of [GAPS](https://huggingface.co/datasets/xavriley/GAPS), the MIT-licensed Guitar-Aligned Performance Scores dataset, into `build/gaps_guitar_samples`, parses the corresponding `.match` files into a GuitarSet-shaped `AUDIO`/`NOTE` manifest, and runs the isolated-guitar note/chord harness. GAPS contains 300 real solo classical-guitar performances, about 14 hours of audio, and aligned MIDI/MusicXML/match data, so the default `GAPS_GUITAR_SAMPLE_LIMIT=4` keeps first runs small; set `GAPS_GUITAR_SAMPLE_LIMIT=0` for the full local sweep. The current cached default gate prepares 4 clips with 3,771 note annotations and passes with 72.94% guitar recall, 67.39% guitar precision, 54.17% exact chord recall, 61.90% exact chord precision, and 84.62% simple major/minor chord recall. The target is included in `make test-real-world-samples-full` as additional real classical-guitar note/chord coverage.
...
`make test-guitar-techs-chord-samples` downloads the P1/P2 Guitar-TECHS chord ZIPs from Zenodo under `build/real_sample_sources/guitar_techs`, extracts MIDI plus DI/amplifier-mic audio, clips candidate chord windows into `build/guitar_techs_chord_samples`, writes a GuitarSet-shaped `AUDIO`/`NOTE` manifest, and runs `analyzer_guitarset` in isolated-guitar mode. The default `GUITAR_TECHS_CHORD_SAMPLE_LIMIT=160` keeps the first pass bounded even though the source chord archives are about 2.2 GB total; set the limit to `0` for a full local sweep after the archives are cached. The current default prepares 160 clips with 564 note annotations, tests 171 selected windows, and passes with 79.60% guitar recall, 84.81% guitar precision, 80.30% exact chord recall, 68.39% exact chord precision, and 77.78% simple major/minor chord recall. Useful knobs include `GUITAR_TECHS_CHORD_MIN_EXCERPTS`, `GUITAR_TECHS_CHORD_MIN_WINDOWS`, `GUITAR_TECHS_CHORD_PERSPECTIVES`, and the `GUITAR_TECHS_CHORD_MIN_*` threshold variables for pitch, guitar-row, and chord recall/precision.
...
`make test-gaps-guitar-samples` is an optional focused analyzer gate for GAPS real classical guitar. It downloads the metadata CSV, a bounded set of WAV performances, and the corresponding `.match` files under `build/gaps_guitar_samples`, converts match-file MIDI tick ranges into `NOTE` rows, and runs `analyzer_guitarset` in isolated-guitar mode. Useful knobs include `GAPS_GUITAR_SAMPLE_LIMIT` (default 4, `0` for all 300 performances), `GAPS_GUITAR_MIN_EXCERPTS`, `GAPS_GUITAR_MIN_WINDOWS`, and the `GAPS_GUITAR_MIN_*` threshold variables for pitch, guitar-row, and chord recall. The default cached analyzer phase currently runs in under one second after downloads are prepared.
