...
- `BPM`: bottom-right estimated tempo. The percentage is confidence from recent weighted drum/onset periodicity across roughly 50-220 BPM, with a rolling onset-flux fallback for weaker broad-band pulses. Body-drum evidence is tracked separately from cymbal/subdivision evidence to reduce half-time and double-time locks, and low half-time candidates can be promoted back to the mid-tempo grid when offbeat cymbals are the clearer repeated pulse. Confidence is capped when the winning tempo mostly comes from long-span grid alignment without direct adjacent-pulse support, so ambiguous half/double/subdivision locks should look less authoritative. Sparse intros, rubato, or weak drums may show `BPM --` or a low-confidence estimate.
