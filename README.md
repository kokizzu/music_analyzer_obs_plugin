...
MUSIC_ANALYZER_URMP_ROOT=/path/to/URMP make inspect-real-urmp
MUSIC_ANALYZER_URMP_ROOT=/path/to/URMP make test-real-urmp
MUSIC_ANALYZER_URMP_ROOT=/path/to/URMP make inspect-real-urmp-full
MUSIC_ANALYZER_URMP_ROOT=/path/to/URMP make test-real-urmp-full
```

`make real-dataset-sources` prints the checked public dataset sources and the exact local URMP commands. The official URMP page routes the full package through dataset download registration, so the project does not attempt an unattended 12.5 GB download. Without `MUSIC_ANALYZER_URMP_ROOT`, the real-data check is built but skipped during normal `make test`; `make test-real-urmp` fails instead, so it can be used as the required gate once the real URMP package is available. `inspect-real-urmp` checks the local dataset layout before analyzer work. `make test` also unpacks the committed URMP-shaped 20-piece lossless FLAC fixture from `tests/fixtures/urmp-mini.tar.gz`, decodes it to disposable WAV files under `build/` with `ffmpeg`, and runs the same harness against it, so the WAV, Notes, MIDI score, and full-mix paths are exercised without downloading URMP. Override the decoder with `FFMPEG=/path/to/ffmpeg` if needed. The fixture is marker-file tagged and rejected by the real-data gate unless `MUSIC_ANALYZER_URMP_ALLOW_GENERATED_FIXTURE=1` is set by the fixture test target. Refresh that fixture with `make update-urmp-fixture` when the fixture generator changes. When pointed at a real URMP dataset, the harness requires official URMP piece folder IDs, reads URMP `AuMix`, `AuSep`, `Notes`, and `Sco` MIDI files, validates that the MIDI score pitch classes agree with the note annotations, requires at least 20 usable same-song pieces and at least 80 annotated note windows by default, checks separated tracks, then checks both the provided `AuMix` and a synthesized mix made by summing all separated tracks. `make test-real-urmp-full` raises that to all 44 official pieces and at least 176 annotated windows. It prints discovered/loadable piece and window counts to diagnose incomplete dataset layouts. Set `MUSIC_ANALYZER_URMP_MAX_WINDOWS_PER_PIECE`, `MUSIC_ANALYZER_URMP_REQUIRED_PIECES`, and `MUSIC_ANALYZER_URMP_REQUIRED_WINDOWS` to lower or raise the coverage gate. Set `MUSIC_ANALYZER_URMP_MIN_WINDOW_RECALL_PERCENT`, `MUSIC_ANALYZER_URMP_MIN_TRACK_RECALL_PERCENT`, `MUSIC_ANALYZER_URMP_MIN_MIX_RECALL_PERCENT`, `MUSIC_ANALYZER_URMP_MIN_CHORD_RECALL_PERCENT`, and `MUSIC_ANALYZER_URMP_MIN_CHORD_CHECKS` to tighten or relax the quality gate.
