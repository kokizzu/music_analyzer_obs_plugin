...
The complete Music Analyzer and compact HalfMusic Analyzer icons were created with ChatGPT image generation. `make icon-assets` regenerates their Android density variants and embedded 64x64 standalone window icons from `assets/music-analyzer-icon.png` and `assets/music-analyzer-bass-guitar-icon.png`.
...
- HalfMusic Analyzer: a smaller renderer that keeps drums, bass, the lower two keyboard bands (C3-B6), guitar, root, and BPM while omitting the lowest keyboard band, vocal, and other rows; its instrument rows use CHORD/SUSTAIN without note-count columns
...
- `BPM`: bottom-right estimated tempo. The percentage is confidence from recent weighted drum/onset periodicity across roughly 50-220 BPM, with rolling onset-flux support for weaker broad-band pulses. Estimates below 15% confidence render as `BPM --`. A small `CAND bpm:score` line above it shows the strongest raw tempo candidates and their scores relative to the current best candidate, so half-time, double-time, or subdivision ambiguity is visible while testing real songs. Body-drum evidence is tracked separately from cymbal/subdivision evidence to reduce half-time and double-time locks, and close body-vs-cymbal events can stay as separate tempo events so offbeat hats do not move the body pulse timestamp. The estimator also folds recent onset flux onto each candidate BPM's beat grid, uses a phase-locked beat recurrence so a candidate needs a coherent continuing pulse, and adds a light four-beat body-meter score from low/downbeat and mid/backbeat evidence. Low half-time candidates can be promoted back to the mid-tempo grid, including cases where weak pre-hit tails make the half-time grid score higher but the double-time grid has equal phase coverage and stronger phase evidence. Weak-phase mid/high subdivision locks can recover to a steadier mid-tempo body grid, and offbeat strum or dense adjacent-pulse locks can yield to a nearby phase-coherent body grid. Mid/high 1.5x or 2x promotions need stronger adjacent-pulse and phase support so loud cymbal subdivisions do not overrule a weaker beat. Confidence is capped when the winning tempo mostly comes from long-span grid alignment without direct adjacent-pulse or phase support, and now capped more aggressively when a half-time, double-time, or 3:2/4:3 harmonic candidate is nearly tied with the displayed tempo. Sparse intros, rubato, or weak drums may show `BPM --` or a low-confidence estimate.
...
- `build/music-analyzer-bass-guitar`: HalfMusic Analyzer with the smaller drums, bass, two-row keyboard, guitar, root, and BPM layout
...
- `bassGuitar`: HalfMusic Analyzer using the smaller drums, bass, two-row keyboard, and guitar layout
...
```sh
make test-parallel
PARALLEL_TEST_JOBS=8 make test-parallel
```
...
For a full detector-tuning pass that runs the parallel regression gates and measured attribute/pattern reports in one coordinated Make job graph with durations:

```sh
make analyze-detector-improvements
PARALLEL_TEST_JOBS=8 MEASURE_ANALYZER_JOBS=8 make analyze-detector-improvements
```

Use `make analyze-detector-improvements-full` when you intentionally want the max real-world sample sweep and exhaustive full-drum pattern reports in the same coordinated parallel pass. Use `make detector-improvement-audit-report` when you intentionally want a fresh route/drum audit. Use `make detector-improvement-status-cached` to re-read the cached coverage shortfalls, route summary, and final audit context without rebuilding sharded analyzer TSVs after local script or Makefile edits. Use `make detector-improvement-coverage-cached` for only the cached coverage shortfall counts and nearest blocked rules, or `make detector-improvement-audit-cached` for only the cached route/audit decision context. After `make measure-analyzer-attribute-rows` has produced the TSV row dumps, use `make print-analyzer-detected-attributes-cached PRINT_ANALYZER_DETECTED_ATTRIBUTES_ARGS=--summary-only` for a compact aggregate read, then use `make measure-analyzer-patterns-cached` or `make detector-improvement-patterns-cached` to rerun the detected-attribute printout and parallel pattern mining from those cached rows without launching the analyzer sample binaries again. Use `make measure-analyzer-patterns-cached-summary` or `make detector-improvement-patterns-cached-summary` when you want the compact ranked actionable/blocked candidate summary from the cached pattern sections plus expanded row-coverage status; use `make measure-analyzer-patterns-cached-coverage` for only that coverage expansion. The cached pattern pass also mines the focused real-note ownership, octave-displacement, strongest-row, visual-row, weak-expected-row, and weak-visual-row sections directly from the existing TSVs.
After a route scan, use `make inspect-detector-coverage-candidates` to inspect cached row TSVs behind coverage-blocked rules; it checks real-note row-confusion candidates, guitar chord coverage blockers, and drum route row blockers without forcing a fresh analyzer run. The output includes aggregate `coverage_status_summary` and nearest-shortfall `nearest_coverage` lines so it is clear whether a blocked detector rule still needs more positive samples or broader row evidence before changing analyzer logic.
...
PARALLEL_TEST_JOBS=8 make test-real-note-samples-full-mix
```

With the current cached 2,212-sample manifest this runs in about 6 seconds with `PARALLEL_TEST_JOBS=4`, versus about 23 seconds for `make test-real-note-samples-full-mix-serial`.

For the generated FluidSynth piano/guitar/bass/synth/strings/vocals/GM-drum gate alone, the default target splits the generated manifests across `PARALLEL_TEST_JOBS` analyzer processes and keeps the combined multi-instrument checks on shard 0:

```sh
make test-instrument-samples
PARALLEL_TEST_JOBS=8 make test-instrument-samples
```

With the current cached generated sample manifests this runs in about 70 seconds with `PARALLEL_TEST_JOBS=4`, versus about 280 seconds for `make test-instrument-samples-serial`.
...
The parallel real-world sweeps use the sharded NSynth full-mix gate instead of the serial full-mix target, so `PARALLEL_TEST_JOBS` applies to that expensive note pass as well as to the independent dataset targets. The Medley-solos timbre gate and MAPS piano chord/music and isolated-note gates also default to sharded analyzer runs; use `make test-medley-solos-samples-serial`, `make test-maps-piano-samples-serial`, or `make test-maps-piano-note-samples-serial` only when comparing against the old single-process output.

The analyzer attribute measurement loop also uses the Makefile fanout for its independent row producers:

```sh
make measure-analyzer-attribute-rows
...
make inspect-drum-sample-coverage
...
make find-protected-drum-full-exact-attribute-patterns
make analyze-mdb-drums-misses
make analyze-star-drums-misses
```

Override `DRUM_SAMPLE_SOURCE_DIR`, `DRUM_SAMPLE_BUILD_DIR`, `DRUM_SAMPLE_LIMIT`, `DRUM_SAMPLE_SELECTION`, `DRUM_SAMPLE_SOURCE_FILTER`, or set `DRUM_SAMPLE_REFRESH=1` if your sample library is elsewhere, you want a different per-category sample count, source-label regex, or selection strategy, or you need to force a rescan of an existing complete manifest. `make inspect-drum-sample-coverage` runs the importer in read-only audit mode and prints classified candidate counts plus the selected counts for the current full-drum spread configuration before you spend time on a detector run. The importer reads plain WAV files, ZIP archives, and RAR archives when `unrar` is installed. The default `DRUM_SAMPLE_SELECTION=spread` local one-shot gate copies up to 160 samples per category, for 1,120 real drum samples when the configured library has enough material, and spreads each category across source folders/archives instead of filling the gate from the first matching directory order. The importer treats compact drum-machine closed/open-hat aliases such as `707oh`, `Realch1`, and `HHOD4`, plus `Hat Pedal`, as hi-hats even when they live under a generic `Cymbals` folder, and treats `Sidestick`, `Side-Stick`, and the observed `Sideststick` spelling as rim/side-stick hits before snare-folder matching. Tom labels now require a real tom token instead of a substring such as `custom`, and unsupported clap/clave/conga-style filenames are skipped unless the filename also identifies a supported snare/rim category. The harness warms the analyzer with a neutral low-level bed instead of pitched notes, credits the expected drum if it appears across the first 50 ms analyzer hop, then keeps unrelated false-positive counts anchored to the onset frame. Rows with that later-frame expected credit are tagged as `merged_expected=1`; the drum pattern finder ignores them by default so detector rules are mined from coherent per-frame attributes unless `--include-merged-rows` is explicitly passed. It prints recall, precision, false positives, a 7x7 active matrix, and a primary matrix that separates no-hit samples from saturated ambiguous strongest-label ties. Set `MUSIC_ANALYZER_DRUM_SAMPLE_SOURCE_SUMMARY=1` to also print the worst source/kit buckets for ordinary hit misses and primary-label misses; for focused debugging, set `MUSIC_ANALYZER_DRUM_SAMPLE_FILTER_CATEGORY`, `MUSIC_ANALYZER_DRUM_SAMPLE_FILTER_SOURCE`, `MUSIC_ANALYZER_DRUM_SAMPLE_VERBOSE_PRIMARY=1`, and `MUSIC_ANALYZER_DRUM_SAMPLE_VERBOSE_ALL=1` to print per-band detector diagnostics for a subset, including shell body ratios, snare crack, upper-tom body, and the selected body-shape family. `make analyze-drum-rule-grid` regenerates full-library kick/snare/tom verbose rows under `build/`, summarizes tom-vs-snare/kick body overlap, prints the current baseline primary counts, and replays candidate tom-promotion rules with per-category and total deltas before changing detector constants. The drum pattern finder can mine either raw verbose logs or TSV attribute dumps created with `scripts/analyze_drum_primary_debug.py --dump-rows`, so a tuning pass can first print the exact detected attributes for each tested sample and then run the same rule search on those rows. Multi-route drum pattern searches run independent routes in parallel by default through `DRUM_PATTERN_JOBS`, which inherits `PARALLEL_TEST_JOBS`; set `DRUM_PATTERN_JOBS=1` or pass `PATTERN_ARGS='--jobs 1 ...'` for serial comparison. Use `make analyze-drum-full-gate-matrix-parallel` to regenerate `build/drum_full_exact_attribute_rows.tsv`, then `make find-protected-drum-full-exact-attribute-patterns` to mine the full rows together with spread, HF drum-kit, and IDMT drum guard rows; `make find-drum-full-exact-attribute-patterns` remains available for full-only diagnostics. The default gate now enforces `MUSIC_ANALYZER_DRUM_SAMPLE_MIN_PRECISION_PERCENT=20`, per-category recall floors for kick/snare/hi-hat/crash/toms/ride/rim of 62/80/88/95/80/88/85 percent, `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_KICK_FALSE_PERCENT=22`, `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_TOM_FALSE_PERCENT=38`, and `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_RIM_FALSE_PERCENT=20`, so kick/snare/cymbal/rim one-shots cannot regress into excessive whole-kit lighting, excessive false kick/tom lighting, or lost side-stick/rim coverage. The default target now fans out one shard per drum category, aggregates the same gates in about 5.3 seconds with a cached manifest, and keeps `make test-drum-samples-serial` for old single-process comparisons. Current default recall/primary/precision is kick 160/160, 155/160, and 160/249; snare 149/160, 126/160, and 149/472; hi-hat 153/160, 135/160, and 153/440; crash 152/160, 105/160, and 152/412; toms 138/160, 118/160, and 138/345; ride 147/160, 103/160, and 147/406; and rim 151/160, 123/160, and 151/330.

`make test-drum-samples-spread` is the faster broad-data tuning loop. It uses deterministic spread selection across plain-WAV source folders and intentionally passes `--no-archives`, so it avoids slow ZIP/RAR extraction on mounted sample libraries. The default target now fans out one shard per drum category and aggregates the same recall, primary-recall, precision, and false-activation thresholds; use `make test-drum-samples-spread-serial` only when comparing the old single-process analyzer output. Override `DRUM_SAMPLE_SPREAD_BUILD_DIR`, `DRUM_SAMPLE_SPREAD_LIMIT`, `DRUM_SAMPLE_SPREAD_MIN_RECALL_PERCENT`, or `DRUM_SAMPLE_SPREAD_MIN_PRECISION_PERCENT` for this target. `make analyze-drum-spread-gate-matrix` builds the exact spread-row TSV through category shards by default, validates the same gate thresholds from the shard outputs, prints durations, and leaves `make analyze-drum-spread-gate-matrix-serial` for comparing the old single-process output. The default spread gate now enforces at least 40% recall, 15% precision, category recall floors for kick/snare/hi-hat/crash/toms/ride/rim of 55/80/90/95/78/88/86 percent, primary-label floors of 90/78/88/68/70/64/78 percent, `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_KICK_FALSE_PERCENT=24`, and `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_TOM_FALSE_PERCENT=45`. On the current local drum library, the bounded spread gate prepares 1,148 real one-shots in about 8 seconds when the manifest is cached: kick=240, snare=240, hi-hat=209, crash=120, tom=240, ride=44, and rim=55. Current spread recall/primary/precision is kick 235/240, 227/240, and 235/351; snare 224/240, 193/240, and 224/489; hi-hat 203/209, 188/209, and 203/409; crash 117/120, 90/120, and 117/283; toms 205/240, 173/240, and 205/441; ride 43/44, 31/44, and 43/201; and rim 53/55, 47/55, and 53/219.

`make test-drum-machine-samples` is a focused local gate over `Roland TR-909 Drum Samples`, `dr202_samples.zip`, and `JazzFunkKit.rar`, selected with `DRUM_MACHINE_SAMPLE_FILTER`. It prepares 358 classic drum-machine/kit one-shots on the current local library: kick=58, snare=154, hi-hat=49, crash=17, tom=61, ride=8, and rim=11. Current recall/primary/precision is kick 58/58, 53/58, and 58/103; snare 130/154, 94/154, and 130/196; hi-hat 47/49, 38/49, and 47/105; crash 15/17, 9/17, and 15/46; toms 43/61, 39/61, and 43/135; ride 7/8, 6/8, and 7/42; and rim 9/11, 0/11, and 9/67. The gate caps tom false activations with `DRUM_MACHINE_MAX_TOM_FALSE_PERCENT=64`; that ceiling is still higher than the aggregate gate because 909/dr202 kick and snare samples are body-heavy and cross-light the tom row more often, but it now guards the current tom reduction instead of hiding it.

`make test-drum-samples-full` uses `DRUM_SAMPLE_FULL_BUILD_DIR`, `DRUM_SAMPLE_FULL_LIMIT`, and the `DRUM_SAMPLE_FULL_MIN_*` thresholds. It is intentionally separate from `make test` because the current local library audit found kick=6,505, snare=5,392, hihat=2,358, crash=788, tom=2,861, ride=352, and rim=504 supported one-shots after filtering unsupported clap/clave/conga-style percussion; two unreadable snare WAVs are skipped by the harness. Complete manifests are reused unless `DRUM_SAMPLE_REFRESH=1` is set, so repeated full sweeps do not re-extract ZIP/RAR members before the analyzer phase. The default target now uses the sharded full-drum gate; use `make test-drum-samples-full-serial` only when you need the old single-process analyzer output. It enforces at least 35% recall, 3% precision, category recall floors for kick/snare/hi-hat/crash/toms/ride/rim of 48/85/94/95/65/90/88 percent, primary-label floors of 91/72/80/60/60/58/64 percent, and `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_TOM_FALSE_PERCENT=45`. Current full-library recall/primary/precision is kick 6,465/6,505, 6,208/6,505, and 6,465/8,425; snare 4,808/5,390, 3,945/5,390, and 4,808/7,869; hi-hat 2,278/2,358, 1,955/2,358, and 2,278/4,620; crash 751/788, 567/788, and 751/2,754; toms 2,143/2,861, 1,840/2,861, and 2,143/6,440; ride 318/352, 221/352, and 318/2,146; and rim 463/504, 337/504, and 463/2,279. Treat this as a broad diagnostic benchmark for improving drum classification, not a precision claim. `make test-real-world-samples-full` also runs `make test-drum-machine-samples` and this full local drum sweep when `DRUM_SAMPLE_SOURCE_DIR` exists, runs E-GMD's 43-kit real-drum analyzer gate when `MUSIC_ANALYZER_EGMD_ROOT` or `EGMD_PATH` is set, and skips those optional local roots otherwise so the full internet-backed benchmark stays portable.
...
make test-instrument-samples-serial
...
make print-analyzer-detected-attributes-cached
make measure-analyzer-detected-attributes
make measure-analyzer-patterns
make measure-analyzer-patterns-cached
make measure-analyzer-patterns-cached-summary
make measure-analyzer-pattern-report
```

This target renders build-local WAV fixtures under `build/piano_samples`, `build/guitar_samples`, `build/bass_samples`, `build/synth_samples`, `build/strings_samples`, `build/vocals_samples`, and `build/drum_kit_samples` using FluidSynth. By default it uses the system FluidR3 GM SoundFont, or downloads/extracts the internet-sourced `fluid-soundfont-gm` package into `build/instrument_sample_sources` if needed. The default fixture target is at least 1000 one-note files per family; the current FluidR3 GM set renders 1024 piano, 1040 guitar, 1030 bass, 1080 synth, 1116 strings, 1020 vocals, and 1043 GM drum-kit samples. The drum-kit fixture now covers the common GM category alternates: kick 35/36, snare 38/40, rim 37, hi-hat 42/44/46, crash 49/57, toms 41/43/45/47/48/50, and ride 51/53/59. The gate currently runs 7,609 checks with a cached analyzer phase of about 258 seconds, including same-pitch C4 source-mode checks across piano, guitar, bass, synth, strings, and vocals plus same-pitch full-mix C and C-E-G chord checks from rendered samples. `make analyze-instrument-sample-attributes` writes `build/instrument_sample_attributes.tsv` with per-note raw pitch rank, +/-cent probe results, full-mix owner/timbre scores, harmonic ratios, spectral centroid/slope/noise, compact full-mix debug candidate lists, and drum trigger details; the exporter uses the same deterministic generated-sample shards as `make test-instrument-samples-parallel` and concatenates the shard TSVs with one header. Current measured generated-note status is 6,310/6,310 hits across bass, guitar, piano, strings, synth, and vocals, plus 1,043 generated GM drum-kit hits. `make print-analyzer-detected-attributes` prints the per-family note ranges directly from the cached row dumps: bass E1-E4, guitar E2-E6, piano C1-C6, strings E1-G6, synth C2-G5, and vocals G3-E5. Measured square-lead synth C2/G3, contrabass E1, high guitar-owned string rows (`tremolo_strings`, `string_ensemble_1`, and `synth_strings_1/2`), and choir/synth-voice full-mix vocal octave recovery now pass. `make inspect-instrument-sample-owner-buckets` groups that TSV by expected family and full-mix debug owner so piano/guitar/synth/string/vocal ownership patterns can be compared before changing detector rules; bass is reported as `bass_debug` because full-mix bass display uses a separate bass path. `make find-instrument-owner-patterns` mines the largest current debug-owner mismatch buckets by default and prints them as `debug_owner_miss:*` so they are not confused with final displayed-note misses. `make find-instrument-status-patterns` mines final displayed-status buckets such as `miss:strings` and `miss:synth`; either target can be focused with `PATTERN_BUCKET='debug_owner_miss:piano->guitar'` or explicit `PATTERN_ARGS` (`owner_miss:*` remains accepted as a legacy alias). `make measure-analyzer-attribute-rows` dumps per-tested-note, per-tested-chord, and spread drum-primary rows from the generated, NSynth, Guitar Chord Mix, and drum fixtures into `build/*_detected_attribute_rows.tsv` and `build/drum_primary_miss_attribute_rows.tsv`; cached `build/instrument_detected_attribute_rows.tsv` is also used as protected evidence in detector route mining, so proposed real-world row-ownership rules must not match known-good generated piano, guitar, bass, synth, string, or vocal notes. The bounded drum-primary TSV includes correct rows as protected examples plus primary misses as positives for pattern mining. `make measure-analyzer-attribute-rows-full` adds the exhaustive protected full-drum rows in `build/drum_full_attribute_rows.tsv`. `make print-analyzer-detected-attributes` refreshes stale derived row dumps from existing cached TSV/log files and prints them without rerunning analyzer binaries, while `make print-analyzer-detected-attributes-cached` runs the same report with no row or manifest prerequisites for a quick read of already-generated files. The report keeps misses sorted before hits and shows the raw pitch, tuning, level, nearest debug candidate, compact miss reason, timbre-score, raw/smoothed/displayed chord, guitar chord match-kind, chord-tone, guitar `evidence_class`/`evidence_source`, direct/melodic probe third-anchor, per-family note ranges, quality recall, and drum-trigger attributes. `make measure-analyzer-detected-attributes` regenerates the bounded row dumps first and then calls the print-only target; `make measure-analyzer-detected-attributes-full` does the same after generating the full-drum TSV. `make measure-analyzer-patterns` regenerates bounded rows, prints them, and mines rule candidates from the same rows, including generated final-status buckets and bounded drum-primary rows. It keeps the active-false drum protection bounded by default, using the HF Drum Kit and IDMT drum primary rows without forcing the exhaustive full-drum TSV refresh. `make measure-analyzer-patterns-cached` reruns that print-and-mine phase from existing cached row dumps, includes the focused real-note ownership, octave, row-confusion, and weak-row pattern sections, and fails fast with the missing path plus `make measure-analyzer-attribute-rows` if a required row file is absent. `make measure-analyzer-patterns-full` is the explicit exhaustive path that also generates and mines protected full-row drum TSVs, and it widens the active-false protected set to include the exact and merged expected full-drum rows. `make measure-analyzer-pattern-report` prints the standard bounded report and saves it as `build/analyzer_measurement_report.txt`; its note sections include compact miss-reason counts, its guitar section includes match-kind/evidence-class counts and median direct/melodic probe third-anchor/opposite-third margins, and its drum route summaries include median detected-minus-expected level margins and trigger-ratio margins so stolen primary labels can be identified before changing thresholds. Use `ATTRIBUTE_ROW_REPORT_ARGS='--rows 50' make print-analyzer-detected-attributes-cached` when you want more direct sample rows from existing TSVs without refreshing prerequisites, or `PATTERN_REPORT_ARGS='--row-examples 25' make measure-analyzer-pattern-report` when you need more concrete examples per mined bucket before changing detector rules; the individual guitar, real-note, and drum pattern finders accept either `--row-examples` or the older `--show-examples` spelling in `PATTERN_ARGS`. Drum pattern output is an attribute selector, not a simulated detector patch; its `foreign`, `new-active`, and conservative `primary-break` counts show non-target rows selected by a candidate, target classes that would be newly activated, and protected primary labels that a route repair could steal. Any drum rule still needs an analyzer rerun with active/primary matrix checks before keeping it. Override `INSTRUMENT_SAMPLE_TARGET_PER_FAMILY`, `INSTRUMENT_SAMPLE_JOBS`, `INSTRUMENT_SAMPLE_SOUNDFONT`, `INSTRUMENT_SAMPLE_SOUNDFONT_PACKAGE`, `INSTRUMENT_SAMPLE_PROGRAMS_PER_FAMILY`, or `INSTRUMENT_SAMPLE_DRUM_KITS` to change the rendered fixture set. Known non-one-note or mislabeled SoundFont cells are excluded from the manifests and recorded in `build/instrument_sample_exclusions.tsv` with reasons; for FluidR3 GM, the orchestra-kit hi-hat cells are excluded because they render as shell-like, non-cymbal audio. FluidR3 GM is MIT-licensed; only generated files under `build/` are written.

The analyzer pattern report is the preferred tuning starting point: inspect the generated TSV attributes first, then mine patterns, then rerun analyzer gates before keeping a detector change. In the guitar chord section, `full-tone expected-label gaps` separates misses where all expected tones are present in the visible, raw-analysis, or smoothed grids from misses where a chord tone is actually absent. `evidence_class` then splits those misses into actionable groups such as `analysis_full_tone_label_gap`, `raw_quality_gap`, `direct_probe_quality_gap`, `melodic_probe_quality_gap`, and `expected_root_absent`, versus intentionally ambiguous `power_only_ambiguous` root+fifth rows where showing major/minor would guess a missing third. `guitar_match_kind` separates exact hits, same-root power/other labels, different-root labels, raw/smoothed-only exact labels, and no-label rows; the guitar pattern miner suppresses outcome-only hit flags so it does not rank tautologies such as `chord_hit<=0`. If full-tone or probe-quality gap counts are non-zero, the next fix should target chord ranking, alias generation, or measured third recovery; if they are zero, the remaining issue is note evidence, tuning, or instrument routing rather than chord naming.
...
make analyze-hf-drum-primary-attribute-rows
make find-hf-drum-primary-attribute-patterns
make analyze-protected-drum-primary-attribute-rows
...
The Hugging Face Drum Kit target defaults to a balanced 300 one-shots per supported class, or 2,100 files across kick, snare, hi-hat, crash, tom, ride, and rim. Its manifest is checkpointed every 50 downloads, so interrupted runs resume safely. Set `HF_DRUM_KIT_LIMIT_PER_CATEGORY=0` only when intentionally downloading the complete upstream corpus.

The Medley-solos, MAPS piano, and Bach10-mf0-synth targets above default to deterministic shard fanout with aggregate corpus-level checks. Tune the fanout with `MEDLEY_SOLOS_SHARDS`, `MAPS_PIANO_SHARDS`, `MAPS_PIANO_NOTE_SHARDS`, `BACH10_MF0_SYNTH_SHARDS`, and `PARALLEL_TEST_JOBS`; use `make test-medley-solos-samples-serial`, `make test-maps-piano-samples-serial`, `make test-maps-piano-note-samples-serial`, or `make test-bach10-mf0-synth-samples-serial` for single-process comparisons. The isolated real-note sample targets also expose direct `*-parallel` entry points, such as `make test-real-note-samples-parallel`, `make test-guitar-fretboard-note-samples-parallel`, and `make test-vocalset-samples-parallel`; those use `REAL_NOTE_SAMPLE_SHARDS`, defaulting from `PARALLEL_TEST_JOBS`. The high-level real-world and detector sample aggregates call direct parallel gates where available, including the optional and max wrappers when their source data is present.
...
Use `make test-real-world-samples-full` for the broad cached benchmark. It runs large optional gates such as Good-sounds, Medley-solos, MAPS, Bach10, and VocalSet only when their archives or source roots are already present under `build/real_sample_sources` or configured by environment variables. Use `make test-real-world-samples-max-parallel` when you intentionally want the largest available internet-backed sample sweep; `make test-real-world-samples-max` is a wrapper for that parallel aggregate. It downloads and runs those optional Good-sounds, Medley-solos, MAPS, Bach10, VocalSet, TinySOL, Guitar-TECHS, GAPS, IDMT, Philharmonia, Iowa, NSynth, drum, and configured-real-dataset gates instead of silently skipping missing archives. The max target also overrides bounded preparation defaults where supported: dedicated full GAPS (`make test-gaps-guitar-samples-full`, writing `build/gaps_guitar_samples_full` with `GAPS_GUITAR_FULL_SAMPLE_LIMIT=0`), unbounded Good-sounds (`GOOD_SOUNDS_SAMPLE_LIMIT=0`), unbounded Medley-solos per-instrument selection (`MEDLEY_SOLOS_LIMIT_PER_INSTRUMENT=0`), all MAPS piano/chord and isolated-note recordings, all Iowa piano candidates, and all discovered Iowa orchestra page ZIPs.

`make test-real-note-samples` downloads the public [NSynth test split](https://magenta.tensorflow.org/datasets/nsynth) into `build/real_sample_sources`, prepares one-note real-audio fixtures under `build/real_note_samples`, and checks exact pitch/instrument-family detection. The current prepared set has 2,212 accepted samples: 137 bass, 346 guitar, 1,117 piano/keyboard, 22 vocal, and 590 other-instrument examples. It excludes out-of-range notes, synthetic-source examples, metadata-marked non-one-note qualities such as `multiphonic` and `tempo-synced`, plus seven exact unstable pitch-reference examples. The gate now enforces per-family minimum coverage with `REAL_NOTE_MIN_BASS`, `REAL_NOTE_MIN_GUITAR`, `REAL_NOTE_MIN_PIANO`, `REAL_NOTE_MIN_VOCALS`, and `REAL_NOTE_MIN_OTHER`, plus strict per-family hit-rate floors with `REAL_NOTE_MIN_BASS_HIT_PERCENT`, `REAL_NOTE_MIN_GUITAR_HIT_PERCENT`, `REAL_NOTE_MIN_PIANO_HIT_PERCENT`, `REAL_NOTE_MIN_VOCALS_HIT_PERCENT`, and `REAL_NOTE_MIN_OTHER_HIT_PERCENT`, all defaulting to 100 percent for the NSynth isolated target. That prevents future dataset-preparation or detector changes from silently collapsing coverage to one family or weakening recall inside a family. The archive is about 333 MB, is cached under `build/`, and the cached parallel analyzer phase currently runs in about 1 second.

`make test-real-note-samples-full-mix` reuses the same NSynth fixtures but analyzes them as a generic full-mix `Speaker Monitor` source. It requires the expected pitch class to appear in any visible note row, scans sustained offsets before deciding expected-family ownership, enforces at least `REAL_NOTE_FULL_MIX_MIN_EXPECTED_ROW_PERCENT` expected-family row ownership (default 80), and caps false drum lighting from melodic one-note audio with `REAL_NOTE_FULL_MIX_MAX_DRUM_ACTIVE_PERCENT` (default 25). It also enforces per-family expected-row floors with `REAL_NOTE_FULL_MIX_MIN_BASS_EXPECTED_ROW_PERCENT`, `REAL_NOTE_FULL_MIX_MIN_GUITAR_EXPECTED_ROW_PERCENT`, `REAL_NOTE_FULL_MIX_MIN_PIANO_EXPECTED_ROW_PERCENT`, `REAL_NOTE_FULL_MIX_MIN_VOCALS_EXPECTED_ROW_PERCENT`, and `REAL_NOTE_FULL_MIX_MIN_OTHER_EXPECTED_ROW_PERCENT`, currently 65/70/85/90/75 percent. `make test-real-note-samples-full-mix-parallel` runs the same corpus in shards, disables whole-corpus percentage gates inside each shard, then enforces those thresholds once through the aggregate shard checker. The aggregate checker also caps the largest measured row and visual source-route confusion buckets, so future tuning cannot silently regress paths such as `piano/electronic->guitar`, `other/acoustic->piano`, `piano/electronic->bass`, `piano/electronic->other`, `other/acoustic->bass`, or `guitar/acoustic->piano`. Current cached local results are 2,212/2,212 full-mix any-row pitch hits, 2,212/2,212 expected-family row hits, and 127/2,903 analyzed sustain windows with any drum row active after tonal sustain suppression. The gate prints expected-row counts by family plus first-visible-row and visual-row confusion matrices, currently showing sustained expected-row hits of bass 137/137, guitar 346/346, piano 1,117/1,117, vocals 22/22, and other 590/590; the cached parallel analyzer phase currently runs in about 1-2 seconds. This gate is included in `make test-real-world-samples` because it catches the generic speaker/music-source path that isolated-source tests do not exercise.
...
`make analyze-real-note-attributes` runs the same NSynth full-mix corpus in diagnostic mode, keeps scanning every sustained offset instead of stopping at the first hit, writes per-buffer detector attributes to `build/real_note_full_mix_attributes.tsv`, and prints a compact summary of status/source/first-row groups plus median ownership scores, pitch confidence, periodicity, fit error, noise, and harmonic partial ratios. The diagnostic TSV uses the same deterministic full-mix shards as `make test-real-note-samples-full-mix-parallel` and concatenates shard TSVs with one header. `make inspect-real-note-attribute-buckets` prints detailed min/quartile/median/max ranges for the largest current hit and miss buckets from that TSV. `make find-real-note-attribute-patterns` mines the largest current real-note miss buckets by default, or a focused bucket with the `PATTERN_BUCKET='status:family/source->first_row'` grammar. For full-mix row ownership, use `make find-real-note-row-confusion-patterns`, `make find-real-note-practical-row-confusion-patterns`, or `make find-real-note-visual-row-confusion-patterns`; these default to detector/runtime attributes and do not auto-mine display-row context fields such as strongest UI row or row levels, because those fields are circular for classifier changes. Use `make find-real-note-focused-row-confusion-patterns` for detector-side route mining with same-source/correct-row protected samples while still excluding UI row context. Use `make find-real-note-focused-visual-row-confusion-patterns` when you intentionally want that UI row context included, for example before deciding whether a visual-row confusion route is a classifier problem or only a display-strength symptom. Real-note bucket mining runs independent buckets in parallel by default through `REAL_NOTE_PATTERN_JOBS`, which inherits `PARALLEL_TEST_JOBS`; set `REAL_NOTE_PATTERN_JOBS=1` or pass `PATTERN_ARGS='--jobs 1 ...'` for serial comparison. Use these before adding new classifier rules: they show measured timbre patterns for hits and misses across the whole tested corpus instead of only the failure snippets.

`make test-vocadito-samples` downloads [Vocadito](https://zenodo.org/records/5578807), a 58.5 MB CC BY 4.0 dataset of 40 short solo monophonic vocal recordings with trained-musician F0, note, lyric, and language annotations. The preparer keeps only A1 note annotations that are long enough for a stable mid-note clip, inside the analyzer vocal range, within `VOCADITO_MAX_CENTS` of a chromatic MIDI note, and whose prepared audio is not dominated by an adjacent pitch class, then writes derived WAV snippets under `build/vocadito_samples`. The current default gate prepares 354 real vocal clips across 27 note names from F2-G#4 and requires at least `VOCADITO_MIN_VOCALS=300` clips. Current local results are 354/354 isolated-vocal detections with zero tolerated misses, and the cached analyzer phase currently runs in about 6 seconds. `make test-vocadito-samples-full-mix-parallel` reuses the same real vocal clips in generic speaker/full-mix mode and checks the current baseline of 354/354 any-row hits, at least 25% vocal-row ownership, at least 5% vocal-first-row ownership, and at most 25% melodic-window drum activity; this target intentionally exposes the current weak full-mix vocal ownership so future detector tuning can improve it without regressing. Override `VOCADITO_ANNOTATOR`, `VOCADITO_SAMPLE_LIMIT`, `VOCADITO_MIN_VOCALS`, `VOCADITO_MAX_CENTS`, `VOCADITO_MIN_NOTE_DURATION`, or the `VOCADITO_FULL_MIX_*` thresholds to tune the sweep.

`make test-vocalset-samples` downloads [VocalSet with corrected annotations](https://zenodo.org/records/10200775), prepares stable one-note clips from the `extended 4` note CSV truth, and runs the same isolated-vocal real-note analyzer gate under `build/vocalset_samples`. This is intentionally an explicit/large optional target: `make test-real-world-samples-full` runs it only when `build/real_sample_sources/vocalset/VocalSet.zip` is already cached, while `make test-real-world-samples-max` downloads it. The default preparer balances up to `VOCALSET_SAMPLE_LIMIT=1200` chromatic sung-note clips, requires `VOCALSET_MIN_VOCALS=800`, filters non-sung or unstable techniques through `VOCALSET_ALLOWED_TECHNIQUES`, and keeps only segments within `VOCALSET_MAX_CENTS=25` of the expected MIDI pitch. `make analyze-vocalset-attributes` now prints the detected-row paths plus the real-note hit/miss summary, and cached VocalSet rows are used as optional candidate and protected evidence in detector route mining when the archive is present.
...
`make test-hf-drum-kit-samples` downloads the public [airasoul/drum-kit](https://huggingface.co/datasets/airasoul/drum-kit) one-shot dataset through the Hugging Face row API, keeps the unambiguous labels that match the analyzer's drum categories, and writes build-local WAV fixtures under `build/hf_drum_kit_samples`. It maps `hat` to hi-hat and skips labels outside the current UI model such as `clap`, `conga`, and generic `cymbal`. The current prepared set has 2,100 real drum one-shots: 300 each for kick, snare, hi-hat, crash, tom, ride, and rim. Reruns validate and reuse a complete cached manifest before touching the Hugging Face API, so a temporary remote `429` does not break an already-prepared local gate. The gate currently enforces a broad diagnostic floor with `HF_DRUM_KIT_MIN_RECALL_PERCENT=20` and `HF_DRUM_KIT_MIN_PRECISION_PERCENT=18`, primary-label floors for kick/snare/hi-hat/crash/tom/ride/rim of 90/96/96/90/92/96/92 percent, and `HF_DRUM_KIT_MAX_KICK_FALSE_PERCENT=12`, then prints the same active and primary matrices as the local drum-sample gates. Current cached local recall/primary/precision is kick 290/300, 286/300, and 290/407; snare 296/300, 295/300, and 296/1340; hi-hat 299/300, 299/300, and 299/1132; crash 293/300, 274/300, and 293/1064; tom 299/300, 291/300, and 299/1088; ride 300/300, 293/300, and 300/1060; and rim 285/300, 284/300, and 285/1095. This gate now passes while still acting as a broad real one-shot diagnostic benchmark for reducing cross-class drum lighting, especially rim/snare/kick/tom confusion. Use `make analyze-protected-drum-primary-attribute-rows` and `make find-protected-drum-primary-attribute-patterns` before changing HF-motivated drum rules; the combined finder compares HF candidates against spread and IDMT protected rows so improvements do not overfit side-stick, snare, or tom-heavy one-shots. Override `HF_DRUM_KIT_LIMIT_PER_CATEGORY` for a smaller loop; use `0` for the full 2,100-sample sweep.

`make test-idmt-drums-samples` downloads [IDMT-SMT-Drums](https://zenodo.org/records/7544164), a 287.1 MB real drum-loop dataset, then extracts annotated isolated training-hit windows from SVL-labelled `#KD#train.wav`, `#SD#train.wav`, and `#HH#train.wav` tracks into `build/idmt_drums_samples`. The current default prepares every usable annotated hit window, currently 1,823 real clips: kick=697, snare=345, and hi-hat=781. The analyzer gate declares only those three classes as required, keeps crash/tom/ride/rim false activations visible as diagnostics, and enforces `IDMT_DRUMS_MIN_RECALL_PERCENT=70`, `IDMT_DRUMS_MIN_SNARE_RECALL_PERCENT=90`, `IDMT_DRUMS_MIN_SNARE_PRIMARY_RECALL_PERCENT=80`, `IDMT_DRUMS_MIN_PRECISION_PERCENT=50`, and `IDMT_DRUMS_MAX_KICK_FALSE_PERCENT=12`. Current cached local recall/primary/precision is kick 696/697, 672/697, and 696/733; snare 336/345, 298/345, and 336/660; and hi-hat 742/781, 677/781, and 742/1109. The cached analyzer phase currently runs in about 46 seconds. This adds real manually annotated kick/snare/hi-hat timing coverage beyond synthetic GM kits and one-shot label folders.

`make test-mdb-drums-samples` downloads the public [MDB Drums](https://github.com/CarlSouthall/MDBDrums) drum-only WAV tracks and subclass onset annotations, converts those annotations into build-local GM drum MIDI files, writes an E-GMD-shaped metadata CSV under `build/mdb_drums_samples`, and reuses the existing E-GMD drum-loop analyzer gate. MDB Drums adds 23 MedleyDB-derived real drum performances with kick, snare, hi-hat, cymbal, tom, ride, rim/side-stick, and brush-style subclass labels. The preparer maps supported subclass labels such as `KD`, `SD`, `SST`, `CHH`, `OHH`, `CRC`, `RDC`, `LFT`, and `MHT` to GM drum MIDI notes, skips unsupported percussion labels such as tambourine, and exposes the skip counts in the prepare log. Useful knobs include `MDB_DRUMS_SOURCE_ROOT` for a local extracted checkout, `MDB_DRUMS_RECORDING_LIMIT` for bounded tuning loops, `MDB_DRUMS_MIN_RECORDINGS` (default 20), `MDB_DRUMS_REQUIRED_WINDOWS` (default 80), `MDB_DRUMS_MIN_RECALL_PERCENT` (default 70 aggregate recall), `MDB_DRUMS_MIN_WINDOW_RECALL_PERCENT` (default 0 because dense human drum-loop frames can have overlapping annotations), `MDB_DRUMS_MIN_PRECISION_PERCENT` (default 55), `MDB_DRUMS_MAX_FALSE_POSITIVE_WINDOWS_PERCENT` (default 70), and `MDB_DRUMS_SHARDS` (default 4). The default target now shards recordings and aggregates the same global coverage, recall, precision, and false-positive-window gates; use `make test-mdb-drums-samples-serial` for old single-process comparisons. Current cached local results on all 23 tracks are 154/192 drum-category hits, 62.10% precision, 80.21% recall, and 63.04% false-positive windows, with per-row recall kick 55/55, snare 38/47, hi-hat 52/59, crash 4/13, tom 1/5, ride 4/12, and rim 0/1; hi-hat and tom over-activation remain the largest diagnostic buckets with 26 and 16 false positives. `make analyze-mdb-drums-misses` reruns this gate with guarded verbose miss/false-positive logging, writes `build/mdb_drums_misses.log`, and summarizes the actionable buckets; use it before changing drum-loop scoring so the miss and false-positive buckets stay tied to current detector attributes. `make analyze-mdb-bpm` prints the same tempo candidate diagnostics for MDB, but its converted annotation MIDI uses a fixed 120 BPM timestamp encoding, so treat that output as candidate evidence rather than BPM ground truth. This is a real drum-loop regression target, not a one-shot purity test, and is included in `make test-real-world-samples`.

`make test-star-drums-samples` downloads the public [STAR Drums](https://zenodo.org/records/15690078) preview ZIP, converts the preview annotations into build-local GM drum MIDI files, converts the selected FLAC audio flavor to WAV, writes an E-GMD-shaped metadata CSV under `build/star_drums_preview_samples`, and reuses the E-GMD drum-loop analyzer gate. The default `STAR_DRUMS_AUDIO_FLAVOR=mix` uses the final mixed-song previews; set it to `re_synthesized_drum` or `original_drum` for drum-stem-only variants when debugging. The preview currently gives 4 recordings and 16 evaluated hit windows; cached local results are 39/56 drum-category hits, 76.47% precision, 69.64% recall, and 62.50% false-positive windows. The default target shards recordings through `STAR_DRUMS_SHARDS` (default 4) and aggregates the original global gate; use `make test-star-drums-samples-serial` for old single-process comparisons. `make analyze-star-drums-misses` writes `build/star_drums_misses.log` and summarizes the mixed-song drum buckets. STAR Drums is valuable mixed-song drum stress because the non-drum audio comes from real melodic/vocal recordings, but its annotated drum stem is re-synthesized from automatic transcription, so it is a diagnostic complement rather than a replacement for real-drum datasets like E-GMD, IDMT-SMT-Drums, or MDB Drums.
...
`make test-medley-solos-samples` downloads the public [Medley-solos-DB](https://zenodo.org/records/3464194) metadata plus the full audio archive, prepares up to `MEDLEY_SOLOS_LIMIT_PER_INSTRUMENT=120` real three-second solo clips per source instrument under `build/medley_solos_samples`, and runs `analyzer_instrument_family_samples` in full-mix mode. The default target shards the manifest and aggregates the original sample-count and per-family recall gates; use `make test-medley-solos-samples-serial` only when comparing the old single-process analyzer output. The target maps distorted electric guitar, piano, and female singer to the guitar, keyboard, and vocal rows, while clarinet, flute, tenor saxophone, trumpet, and violin map to `other`. Medley-solos has instrument labels but no pitch/chord labels, so this gate checks broad timbre row activity and prints cross-row diagnostics; it is not a note or chord accuracy gate. The audio archive is about 7.9 GB, so `make test-real-world-samples-full` runs this gate only when `build/real_sample_sources/medley_solos/Medley-solos-DB.tar.gz` is already cached. Use `MEDLEY_SOLOS_MIN_RECALL_PERCENT`, `MEDLEY_SOLOS_MIN_SAMPLES`, `MEDLEY_SOLOS_MIN_COUNTS`, and `MEDLEY_SOLOS_SHARDS` when tuning the row-routing floor or shard fanout.

`make test-downloaded-guitarset` downloads [GuitarSet](https://zenodo.org/records/3371780) annotations plus the mono microphone audio into `build/real_sample_sources/guitarset`, prepares `build/guitarset-manifest.tsv`, and runs isolated-guitar note/chord regression over all usable annotated chord windows through `make test-downloaded-guitarset-parallel`.
...
Current local results are 4,413/5,451 pitch-class hits, 66.52% guitar precision, 80.96% guitar recall, 73.03% guitar F1, and 1,089/1,491 exact chord hits: 663/752 major/minor opportunities and 426/739 other-chord opportunities.
The same run reports 1,203/1,491 simplified-compatible chord hits, including 669/752 major/minor opportunities and 534/739 other-chord opportunities; this distinguishes missed extensions from completely wrong chord roots. The current gate has 402 remaining exact chord misses.
...
The quality breakdown currently shows major 458/500 exact and 463/500 simplified, minor 205/255 exact and 207/255 simplified, sus2 52/88, sus4 42/88, 7th 100/160 exact and 118/160 simplified, maj7 99/202 exact and 142/202 simplified, m7 105/173 exact and 116/173 simplified, 9th 15/38 exact and 25/38 simplified, add9 32/61 exact and 42/61 simplified, 6th 106/173 exact and 129/173 simplified, m6 29/57 exact and 33/57 simplified, dim 12/26, m7b5 30/57, power 1/2, maj9 2/6 exact and 3/6 simplified, aug 9/27, dim7 0/8, and augmented/diminished-seventh shapes still weak.
...
`make test-guitar-techs-chord-samples` downloads the public Guitar-TECHS P1/P2 chord archives, prepares `build/guitar_techs_chord_samples/manifest.tsv`, and runs the GuitarSet-shaped isolated-guitar chord harness over the real chord clips through `make test-guitar-techs-chord-samples-parallel`. The current cached gate covers 7,016 excerpts / 7,788 windows, with 24,497/27,650 note hits and 6,735/7,484 exact chord hits. Use `make analyze-guitar-techs-chord-attributes` to write `build/guitar_techs_chord_attributes.tsv` with displayed, analysis, smoothed, and raw chord evidence for every checked window, then `make analyze-guitar-techs-chord-extra-components` to summarize crowded candidate labels and simulated prune policies. The current Guitar-TECHS chord attribute sweep shows many expected non-major/minor qualities: 7th, 6/m7, m6/m7b5, maj7, diminished, and augmented rows are all represented. It also shows that a plain-only guitar chord prune would be unsafe for this dataset, losing 4,048 current exact chord hits, mostly extended or altered qualities, so chord-label pruning must preserve supported common-chord aliases instead of reducing the output to only major/minor labels.

`make test-guitar-chord-mix-samples` downloads all currently matched public [Guitar Chord Mix](https://huggingface.co/datasets/ryangowe/guitar-chord-mix) WAV/JAMS clips into `build/guitar_chord_mix_samples`, writes a GuitarSet-shaped manifest from the per-string `note_midi` annotations, and runs the isolated-guitar note/chord harness through the sharded gate by default. Use `make test-guitar-chord-mix-samples-serial` only when you need the old single-process analyzer output. The default `GUITAR_CHORD_MIX_LIMIT=0` keeps the gate on the full available matched set; set a positive limit only when you need a smaller local tuning loop. The current cached default gate prepares 500 clips with 2,552 note annotations across 16 chord labels, tests 511 windows, and requires at least 480 exact chord hits plus at least 400 primary chord hits so the expected chord cannot regress into only a later multi-label alias. It currently passes with 82.84% guitar recall, 67.84% guitar precision, 485/511 exact chord hits, 94.91% exact chord recall, 96.61% exact chord precision, and 400/511 primary chord hits. The remaining misses are dominated by root+fifth-heavy real-guitar windows where the annotated major/minor third is too weak to justify guessing a quality, so the analyzer keeps those as `pow` unless grid or consistent direct/melodic probe evidence is stronger. This is real guitar chord audio coverage and is included in `make test-real-world-samples-full`. Use `make analyze-guitar-chord-mix-attributes` before changing guitar chord detection: it writes `build/guitar_chord_mix_attributes.tsv` with expected pitch classes, displayed guitar grid, raw analysis grid, smoothed grid, raw expected-tone levels, internal raw direct/melodic probe levels, raw/smoothed/displayed chord labels, and detected chords. `make analyze-guitar-chord-mix-recovery` then prints evidence-class/source counts, which remaining chord misses still have root+fifth support in the visible, analysis, smoothed, and combined grids, plus raw direct-probe, raw melodic-probe, and test-raw promotion threshold simulations for same-root power-dyad recovery. Then use `make inspect-guitar-chord-mix-attribute-buckets` to print min/quartile/median/max ranges for the largest chord-hit and chord-miss support buckets. `make find-guitar-chord-mix-attribute-patterns` mines the largest current chord-miss buckets by default; pass `PATTERN_BUCKET='chord_miss:maj:visible2_analysis2_smooth2_rootvis1'` for a focused search against protected passing rows.

`make test-egfxset-guitar-samples` reuses the same Hugging Face Guitar Chord Mix WAV/JAMS preparer against its `egfxset` subtree, writes `build/egfxset_guitar_samples/manifest.tsv`, and runs the GuitarSet-shaped harness as an isolated-guitar single-note gate through `make test-egfxset-guitar-samples-parallel`. The default `EGFXSET_GUITAR_SAMPLE_LIMIT=0` prepares every currently matched EGFxSet clip, with `EGFXSET_GUITAR_MIN_EXCERPTS=490` allowing a small remote-data cushion. The preparer supports `--jobs` and this target defaults `EGFXSET_GUITAR_DOWNLOAD_JOBS=8`, while preserving deterministic manifest order. The current cached gate prepares 493 real effected electric-guitar clips with one note annotation each, covers 493 windows, and passes with 493/493 pitch-class hits, 100.00% guitar recall, 70.53% guitar precision, 82.72% guitar F1, no cross-row contamination, and no false vocal windows. The remaining extra same-row harmonic pitch classes from distorted/effected single notes stay visible as a crowding diagnostic; the gate permits at most 1% false single-note chord labels (`EGFXSET_GUITAR_MAX_SINGLE_NOTE_CHORD_FALSE_PERCENT=1`, currently 4/493 or 0.81%).

`make test-gaps-guitar-samples` downloads a bounded subset of [GAPS](https://huggingface.co/datasets/xavriley/GAPS), the MIT-licensed Guitar-Aligned Performance Scores dataset, into `build/gaps_guitar_samples`, prefilters the Hugging Face `match/` tree so unavailable annotations are skipped before download attempts, parses the corresponding `.match` files into a GuitarSet-shaped `AUDIO`/`NOTE` manifest, and runs the isolated-guitar note/chord harness through `make test-gaps-guitar-samples-parallel`. GAPS contains 300 real solo classical-guitar performances, about 14 hours of audio, and aligned MIDI/MusicXML/match data. The default `GAPS_GUITAR_SAMPLE_LIMIT=42` prepares 40+ usable excerpts; use `make test-gaps-guitar-samples-full` / `make test-gaps-guitar-samples-full-parallel` for the full available local sweep in `build/gaps_guitar_samples_full`. The current cached default gate prepares 42 clips, skips about 130 unavailable match rows up front, tests 41 usable excerpts / 246 windows, and passes with 73.99% guitar recall, 65.18% guitar precision, 139/244 exact chord hits, 56.97% exact chord recall, and 59.40% exact chord precision. The target is included in `make test-real-world-samples-full` as additional real classical-guitar note/chord coverage.
...
`make test-guitar-techs-chord-samples` downloads the P1/P2 Guitar-TECHS chord ZIPs from Zenodo under `build/real_sample_sources/guitar_techs`, extracts MIDI plus DI/amplifier-mic audio, clips candidate chord windows into `build/guitar_techs_chord_samples`, writes a GuitarSet-shaped `AUDIO`/`NOTE` manifest, and runs `analyzer_guitarset` in isolated-guitar mode through `make test-guitar-techs-chord-samples-parallel`. The default `GUITAR_TECHS_CHORD_SAMPLE_LIMIT=0` uses the full current P1/P2 chord sweep; set a positive limit only for a smaller local tuning loop. The current public archives prepare 3,804 clips with 13,474 note annotations and test 4,338 windows. The full gate passes with 92.91% guitar recall, 89.14% guitar precision, 95.17% exact chord recall, and 90.13% exact chord precision. Useful knobs include `GUITAR_TECHS_CHORD_MIN_EXCERPTS` (default 3000), `GUITAR_TECHS_CHORD_MIN_WINDOWS` (default 3000), `GUITAR_TECHS_CHORD_PERSPECTIVES`, and the `GUITAR_TECHS_CHORD_MIN_*` threshold variables for pitch, guitar-row, and chord recall/precision. When the cached manifest is present, detector route scans also mine `build/guitar_techs_chord_attributes.tsv` through `find-guitar-techs-chord-route-patterns` so Guitar-TECHS chord misses can unblock or reject guitar chord rule candidates. The source chord archives are about 2.2 GB total; the cached analyzer phase currently runs in a few seconds with the 32-shard fanout.

`make test-guitar-chord-mix-samples` is an optional focused analyzer gate for real guitar chords. It downloads Hugging Face Guitar Chord Mix WAV/JAMS clips under `build/guitar_chord_mix_samples`, prepares a GuitarSet-shaped manifest from the six per-string `note_midi` annotations, and runs `analyzer_guitarset` in isolated-guitar mode. Useful knobs include `GUITAR_CHORD_MIX_LIMIT` (default `0` for all matched pairs; use a positive number for a bounded local loop), `GUITAR_CHORD_MIX_MIN_EXCERPTS` (default 500), `GUITAR_CHORD_MIX_MIN_WINDOWS` (default 500), and the `GUITAR_CHORD_MIX_MIN_*` threshold variables for pitch, guitar-row, chord recall, and primary chord hits. The current full gate prepares 500 clips / 511 windows, requires at least 480 exact chord hits and 400 primary chord hits, passes with 483/511 exact chord hits, 96.22% exact chord precision, 94.52% exact chord recall, 400/511 primary chord hits, and 83 expected chords later than primary; with a cached manifest, the 32-shard analyzer phase runs in about 0.26 seconds and the aggregate checker in about 0.09 seconds. `make analyze-guitar-chord-mix-misses` writes verbose misses to `build/guitar_chord_mix_misses.log`; `make analyze-guitar-chord-mix-attributes` writes detector attributes to `build/guitar_chord_mix_attributes.tsv` through the same `PARALLEL_TEST_JOBS` shard fanout as the gate, and `make analyze-guitar-chord-mix-recovery` prints evidence classes/sources, root+fifth support, raw direct-probe, raw melodic-probe, and test-raw promotion threshold candidates remaining in missed rows. Use the attribute TSV and pattern targets above before changing guitar chord detection so candidate changes can be checked against protected passing rows.

`make test-gaps-guitar-samples` is an optional focused analyzer gate for GAPS real classical guitar. It downloads the metadata CSV, prefilters the Hugging Face `match/` tree so unavailable annotations are skipped before download attempts, downloads a bounded set of WAV performances plus corresponding `.match` files under `build/gaps_guitar_samples`, converts match-file MIDI tick ranges into `NOTE` rows, and runs `analyzer_guitarset` in isolated-guitar mode through `make test-gaps-guitar-samples-parallel`. Useful knobs include `GAPS_GUITAR_SAMPLE_LIMIT` (default 42 prepared clips), `GAPS_GUITAR_MIN_EXCERPTS` (default 40 usable excerpts), `GAPS_GUITAR_MIN_WINDOWS`, `GAPS_GUITAR_MATCH_TREE_JSON`, `GAPS_GUITAR_NO_MATCH_TREE=1`, and the `GAPS_GUITAR_MIN_*` threshold variables for pitch, guitar-row, and chord recall. Use `make test-gaps-guitar-samples-full` / `make test-gaps-guitar-samples-full-parallel` for the all-available sweep in `build/gaps_guitar_samples_full`; its knobs are `GAPS_GUITAR_FULL_SAMPLE_LIMIT` (default `0`), `GAPS_GUITAR_FULL_MIN_EXCERPTS`, and `GAPS_GUITAR_FULL_MIN_WINDOWS`, and `make test-real-world-samples-max` runs this full target. The current default prepares 42 clips, skips about 130 unavailable match rows up front, tests 41 usable excerpts / 246 windows after the cache is populated, passes with 139/244 exact chord hits, and keeps the analyzer phase under 5 seconds. `make analyze-gaps-guitar-misses` writes verbose misses to `build/gaps_guitar_misses.log`, and `make analyze-gaps-guitar-misses-full` does the same for the full manifest.
