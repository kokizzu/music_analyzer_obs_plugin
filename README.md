...
Override `DRUM_SAMPLE_SOURCE_DIR`, `DRUM_SAMPLE_BUILD_DIR`, `DRUM_SAMPLE_LIMIT`, or `DRUM_SAMPLE_SELECTION` if your sample library is elsewhere or you want a different per-category sample count or selection strategy. The importer reads plain WAV files, ZIP archives, and RAR archives when `unrar` is installed. The default `DRUM_SAMPLE_SELECTION=first` local one-shot gate copies up to 160 samples per category, for 1,120 real drum samples when the configured library has enough material, and stops scanning once every category reaches the configured limit. Use `DRUM_SAMPLE_SELECTION=spread` for a deeper deterministic round-robin across source folders/archives; this can be much slower on large mounted libraries, and `UNRAR=` disables RAR extraction for a plain/ZIP-only audit. The harness warms the analyzer with a neutral low-level bed instead of pitched notes, then prints recall, precision, false positives, and a 7x7 active matrix. The default gate also enforces `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_TOM_FALSE_PERCENT=55`, `MUSIC_ANALYZER_DRUM_SAMPLE_MIN_RIM_RECALL_PERCENT=73`, and `MUSIC_ANALYZER_DRUM_SAMPLE_MAX_RIM_FALSE_PERCENT=20`, so kick/snare/cymbal/rim one-shots cannot regress into excessive false tom lighting or lose the side-stick/rim coverage. The current default gate runs in about 25.29 seconds and passes with recall/precision kick 103/160 and 103/379, snare 91/160 and 91/483, hi-hat 101/160 and 101/595, crash 72/160 and 72/493, toms 95/160 and 95/622, ride 72/160 and 72/482, and rim 118/160 and 118/277.
