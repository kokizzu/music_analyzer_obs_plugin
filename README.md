...
The Medley-solos and MAPS piano targets above default to deterministic shard fanout with aggregate corpus-level checks. Tune the fanout with `MEDLEY_SOLOS_SHARDS`, `MAPS_PIANO_SHARDS`, `MAPS_PIANO_NOTE_SHARDS`, and `PARALLEL_TEST_JOBS`; use `make test-medley-solos-samples-serial`, `make test-maps-piano-samples-serial`, or `make test-maps-piano-note-samples-serial` for single-process comparisons.
