...
- `BPM`: bottom-right estimated tempo. The percentage is confidence from recent weighted drum/onset periodicity across roughly 50-220 BPM, with rolling onset-flux support for weaker broad-band pulses. Body-drum evidence is tracked separately from cymbal/subdivision evidence to reduce half-time and double-time locks. Low half-time candidates can be promoted back to the mid-tempo grid, but mid/high 1.5x or 2x promotions now need stronger adjacent-pulse support so loud cymbal subdivisions do not overrule a weaker beat. Confidence is capped when the winning tempo mostly comes from long-span grid alignment without direct adjacent-pulse support, so ambiguous half/double/subdivision locks should look less authoritative. Sparse intros, rubato, or weak drums may show `BPM --` or a low-confidence estimate.
