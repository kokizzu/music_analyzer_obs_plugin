...
Use `make test-real-world-samples-full` for the broad cached benchmark. It runs large optional gates such as Good-sounds, Medley-solos, MAPS, and Bach10 only when their archives or source roots are already present under `build/real_sample_sources` or configured by environment variables. Use `make test-real-world-samples-max` when you intentionally want the largest available internet-backed sample sweep; it downloads and runs those optional Good-sounds, Medley-solos, MAPS, Bach10, TinySOL, Guitar-TECHS, GAPS, IDMT, Philharmonia, Iowa, NSynth, drum, and configured-real-dataset gates instead of silently skipping missing archives.
