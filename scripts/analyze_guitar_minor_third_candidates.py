#!/usr/bin/env python3
"""Sweep compact minor-triad third recovery evidence in guitar attribute TSVs."""
...
def plain_minor(component: str) -> bool:
    root = chord_root(component)
    return bool(root) and component[len(root) :] == "m"


def collect(path: pathlib.Path) -> list[tuple[bool, float, float, dict[str, str]]]:
    rows: list[tuple[bool, float, float, dict[str, str]]] = []
    for row in load_rows(path):
        labels = split_chord_components(row.get("guitar_chord", ""))
        if len(labels) != 1 or not plain_minor(labels[0]):
            continue
