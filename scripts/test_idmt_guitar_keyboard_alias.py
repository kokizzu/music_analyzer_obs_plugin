...
GUITAR_FS4_ID = "idmt_guitar_FS_Lick2_KVSH_0005_F#4_PK_SL"
...
def main() -> int:
    ...
    guitar_fs4_lines = debug_lines(debug_output(GUITAR_FS4_ID))
    if not guitar_fs4_lines:
        raise RuntimeError("missing F#4 guitar debug output")
    if not any(row_has_note(line, "guitar", "F#4") for line in guitar_fs4_lines):
        raise RuntimeError("IDMT F#4 distorted guitar is not recovered in Guitar")
    ...
    print(f"idmt-guitar-keyboard-alias: E2/G2/G#3/F#4 reach Guitar; {piano_id} retains Keyboard {piano_note}")
    return 0
