#!/usr/bin/env python3
"""Validate the public ESMUC Choir Dataset archive before extraction."""
...
def validate(path: Path, expected_md5: str) -> int:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        bad = [entry.filename for entry in members if not safe_member(entry.filename)]
        if bad:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"validate_esmuc_choir_dataset: valid {args.archive} members={members}")
    return 0
