#!/usr/bin/env python3
"""Measure a deterministic Vocal-family shard with per-note ownership attributes."""
...
def main() -> int:
    ...
    wrong = [
        ...
        if row["first_row"] == "vocals" and row["visual_first_row"] != "vocals"
    ]
    print(f"fixtures={len(samples)} raw={dict(raw)} visual={dict(visual)}")
    print(f"raw-vocals-visual-other={len(wrong)}")
    for row in wrong[:12]:
        print(
            f"  {row['sample_id']} {row['expected_note']} visual={row['visual_first_row']} "
            f"vocal={row['vocal_visual_level']} piano={row['piano_visual_level']} "
            f"bass={row['bass_visual_level']} guitar={row['guitar_visual_level']} "
            f"other={row['other_visual_level']} debug={row['debug_note']}:{row['debug_owner']} "
            f"scores=k{row['keyboard_score']},g{row['guitar_score']},v{row['vocal_score']},o{row['other_score']}"
        )
    return 0
