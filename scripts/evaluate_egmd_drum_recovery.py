...
def broad_low_dominant_ride(event: DrumEvent) -> bool:
    ride_seg = value(event, "ride", "seg")
    cymbal_max = strongest(event, ("hihat", "crash", "ride"), "seg")
    return (
        10.0 <= trigger_ratio(event, "ride") <= 11.0
        and 2.0 <= value(event, "ride", "transient") <= 2.2
        and 16.0 <= value(event, "ride", "onset") <= 18.0
        and 0.11 <= value(event, "ride", "rms") <= 0.12
        and 0.86 <= value(event, "ride", "low") <= 0.90
        and 0.08 <= value(event, "ride", "mid") <= 0.10
        and 0.02 <= value(event, "ride", "high") <= 0.04
        and 3.0 <= ride_seg <= 3.2
        and ride_seg >= cymbal_max * 0.95
    )


def mid_heavy_ride(event: DrumEvent) -> bool:
    ride_seg = value(event, "ride", "seg")
    cymbal_max = strongest(event, ("hihat", "crash", "ride"), "seg")
    return (
        8.0 <= trigger_ratio(event, "ride") <= 10.0
        and 1.80 <= value(event, "ride", "transient") <= 1.95
        and 30.0 <= value(event, "ride", "onset") <= 35.0
        and 0.080 <= value(event, "ride", "rms") <= 0.090
        and 0.15 <= value(event, "ride", "low") <= 0.20
        and 0.55 <= value(event, "ride", "mid") <= 0.65
        and 0.20 <= value(event, "ride", "high") <= 0.28
        and 2.1 <= ride_seg <= 2.5
        and ride_seg >= cymbal_max * 0.95
    )


def compact_long_onset_ride(event: DrumEvent) -> bool:
    ride_seg = value(event, "ride", "seg")
    cymbal_max = strongest(event, ("hihat", "crash", "ride"), "seg")
    return (
        4.0 <= trigger_ratio(event, "ride") <= 5.0
        and 2.3 <= value(event, "ride", "transient") <= 2.5
        and 20.0 <= value(event, "ride", "onset") <= 23.0
        and 0.035 <= value(event, "ride", "rms") <= 0.045
        and 0.84 <= value(event, "ride", "low") <= 0.86
        and 0.11 <= value(event, "ride", "mid") <= 0.15
        and 0.01 <= value(event, "ride", "high") <= 0.03
        and 0.55 <= ride_seg <= 0.65
        and ride_seg >= cymbal_max * 0.95
    )
...
def compact_long_segment_snare(event: DrumEvent) -> bool:
    return (
        4.0 <= trigger_ratio(event, "snare") <= 5.0
        and 1.85 <= value(event, "snare", "transient") <= 2.05
        and 6.0 <= value(event, "snare", "onset") <= 8.0
        and 0.055 <= value(event, "snare", "rms") <= 0.070
        and 0.84 <= value(event, "snare", "low") <= 0.86
        and 0.10 <= value(event, "snare", "mid") <= 0.12
        and 0.03 <= value(event, "snare", "high") <= 0.05
        and 28.0 <= value(event, "snare", "seg") <= 31.0
    )


def threshold_mid_heavy_snare(event: DrumEvent) -> bool:
    return (
        1.10 <= trigger_ratio(event, "snare") <= 1.15
        and 1.60 <= value(event, "snare", "transient") <= 1.70
        and 2.5 <= value(event, "snare", "onset") <= 3.0
        and 0.065 <= value(event, "snare", "rms") <= 0.080
        and 0.65 <= value(event, "snare", "low") <= 0.73
        and 0.22 <= value(event, "snare", "mid") <= 0.28
        and 0.04 <= value(event, "snare", "high") <= 0.08
        and 30.0 <= value(event, "snare", "seg") <= 33.0
    )
...
def quiet_threshold_tom(event: DrumEvent) -> bool:
    return (
        2.2 <= trigger_ratio(event, "tom") <= 2.4
        and 1.35 <= value(event, "tom", "transient") <= 1.50
        and 2.5 <= value(event, "tom", "onset") <= 3.0
        and 0.040 <= value(event, "tom", "rms") <= 0.050
        and 0.88 <= value(event, "tom", "low") <= 0.92
        and 0.06 <= value(event, "tom", "mid") <= 0.10
        and 0.01 <= value(event, "tom", "high") <= 0.03
        and 29.0 <= value(event, "tom", "seg") <= 32.0
    )


def dense_mixed_rim(event: DrumEvent) -> bool:
    return (
        15.0 <= trigger_ratio(event, "rim") <= 17.0
        and 1.80 <= value(event, "rim", "transient") <= 1.95
        and 27.0 <= value(event, "rim", "onset") <= 30.0
        and 0.060 <= value(event, "rim", "rms") <= 0.080
        and 0.68 <= value(event, "rim", "low") <= 0.74
        and 0.13 <= value(event, "rim", "mid") <= 0.17
        and 0.12 <= value(event, "rim", "high") <= 0.16
        and 5.0 <= value(event, "rim", "seg") <= 5.4
    )
...
RULES = (
    ...
    CandidateRule("quiet-threshold-tom", "tom", quiet_threshold_tom),
    CandidateRule("dense-mixed-rim", "rim", dense_mixed_rim),
)
