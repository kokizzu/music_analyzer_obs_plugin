...
DETECTION_ACCURACY_URMP_GOOD_SOUNDS_SAX_SHARED_PATTERN_ARG = --urmp-good-sounds-sax-shared-pattern-audit "$(URMP_GOOD_SOUNDS_SAX_SHARED_PATTERN_REPORT)" $(DETECTION_ACCURACY_OCTAVE_CORRECTION_CROSS_CORPUS_AUDIT_ARG) $(DETECTION_ACCURACY_DOMINANT_SEVENTH_EXTENSION_AUDIT_ARG)
...
DETECTION_ACCURACY_KRAISLER_MEASUREMENT_ARG = $(if $(wildcard $(KRAISLER_MEASUREMENT_OUTPUT)),--kraisler-measurement "$(KRAISLER_MEASUREMENT_OUTPUT)")
...
DETECTION_ACCURACY_STAR_DRUMS_GATE_OUTPUT ?= $(BUILD_DIR)/star_drums_misses.log.summary
...
DETECTION_ACCURACY_MDB_DRUMS_GATE_OUTPUT ?= $(MDB_DRUMS_MISS_LOG).summary
...
update-detection-accuracy-report-cached: scripts/write_detection_accuracy_report.py
	...
	@for path in "$(GUITAR_CHORD_PRIMARY_DISPLAY_AUDIT)" "$(GUITAR_CHORD_TONE_RECOVERY_AUDIT)" "$(URMP_GOOD_SOUNDS_SAX_SHARED_PATTERN_REPORT)" "$(OCTAVE_CORRECTION_CROSS_CORPUS_AUDIT)" "$(DOMINANT_SEVENTH_EXTENSION_AUDIT)" "$(GLOBAL_CHORD_CONFIDENCE_AUDIT)" "$(SAME_ROOT_GUITAR_QUALITY_AUDIT)" "$(OWNER_CLASSIFIER_LOCO_AUDIT)"; do test -s "$$path" || { printf '%s\n' "missing cached accuracy audit: $$path"; exit 2; }; done
...
.PHONY: evaluate-owner-classifier-loco test-evaluate-owner-classifier-loco
evaluate-owner-classifier-loco: scripts/evaluate_owner_classifier_loco.py
	@for path in "$(BUILD_DIR)/real_note_full_mix_attributes.tsv" "$(GOOD_SOUNDS_FULL_MIX_ATTRIBUTE_TSV)" "$(PHILHARMONIA_FULL_ATTRIBUTE_TSV)" "$(IOWA_ORCHESTRA_FULL_ATTRIBUTE_TSV)"; do test -s "$$path" || { printf '%s\n' "missing cached owner-classifier input: $$path"; exit 2; }; done
	@tmp="$(OWNER_CLASSIFIER_LOCO_AUDIT).$$$$.tmp"; $(PYTHON) scripts/evaluate_owner_classifier_loco.py "$(BUILD_DIR)/real_note_full_mix_attributes.tsv" "$(GOOD_SOUNDS_FULL_MIX_ATTRIBUTE_TSV)" "$(PHILHARMONIA_FULL_ATTRIBUTE_TSV)" "$(IOWA_ORCHESTRA_FULL_ATTRIBUTE_TSV)" > "$$tmp" && mv "$$tmp" "$(OWNER_CLASSIFIER_LOCO_AUDIT)" && cat "$(OWNER_CLASSIFIER_LOCO_AUDIT)"
...
.PHONY: inspect-high-vocal-octave-evidence-cached test-inspect-high-vocal-octave-evidence
