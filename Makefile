...
.PHONY: find-real-note-row-confusion-patterns
...
.PHONY: analyze-guitar-chord-mix-recovery analyze-guitar-chord-mix-extra-components test-guitar-chord-recovery-analysis test-guitar-chord-extra-components-analysis test-guitar-chord-mix-samples-parallel $(GUITAR_CHORD_MIX_SHARD_TARGETS)
...
$(BUILD_DIR)/instrument_sample_attributes.tsv: $(BUILD_DIR)/analyzer_instrument_samples $(INSTRUMENT_SAMPLE_MANIFEST_STAMP) scripts/prepare_instrument_samples.py | $(BUILD_DIR)
	$(MAKE) $(INSTRUMENT_SAMPLE_ATTRIBUTE_MAKE_JOBS) $(INSTRUMENT_SAMPLE_ATTRIBUTE_PARTS)
	awk 'FNR == 1 && NR != 1 { next } { print }' $(INSTRUMENT_SAMPLE_ATTRIBUTE_PARTS) > "$@.tmp"
	mv "$@.tmp" "$@"
...
$(BUILD_DIR)/real_note_full_mix_attributes.tsv: $(BUILD_DIR)/analyzer_real_note_samples scripts/prepare_nsynth_samples.py | $(BUILD_DIR)
	@if [ ! -f "$(REAL_NOTE_SAMPLE_DIR)/manifest.tsv" ] || [ "scripts/prepare_nsynth_samples.py" -nt "$(REAL_NOTE_SAMPLE_DIR)/manifest.tsv" ]; then $(MAKE) prepare-real-note-samples; fi
	$(MAKE) $(REAL_NOTE_FULL_MIX_ATTRIBUTE_MAKE_JOBS) $(REAL_NOTE_FULL_MIX_ATTRIBUTE_PARTS)
	awk 'FNR == 1 && NR != 1 { next } { print }' $(REAL_NOTE_FULL_MIX_ATTRIBUTE_PARTS) > "$@.tmp"
	mv "$@.tmp" "$@"
...
test-guitar-chord-mix-samples-parallel: $(BUILD_DIR)/analyzer_guitarset prepare-guitar-chord-mix-samples scripts/run_with_duration.sh
	$(MAKE) $(GUITAR_CHORD_MIX_TEST_MAKE_JOBS) $(GUITAR_CHORD_MIX_SHARD_TARGETS)

test-guitar-chord-mix-samples-shard-%: FORCE $(BUILD_DIR)/analyzer_guitarset prepare-guitar-chord-mix-samples scripts/run_with_duration.sh
