...
.PHONY: measure-analyzer-attributes measure-analyzer-attribute-rows measure-analyzer-patterns inspect-instrument-sample-owner-buckets find-instrument-owner-patterns test-instrument-sample-owner-buckets test-instrument-owner-patterns test-analyzer-pattern-report analyze-drum-tom-bleed-caps
...
find-drum-attribute-patterns: scripts/find_drum_attribute_patterns.py
	@if [ ! -f "$(BUILD_DIR)/full_kick_debug.err" ] || [ ! -f "$(BUILD_DIR)/full_snare_debug.err" ] || [ ! -f "$(BUILD_DIR)/full_tom_debug.err" ]; then $(MAKE) analyze-drum-rule-grid; fi
	$(PYTHON) scripts/find_drum_attribute_patterns.py "$(BUILD_DIR)/full_kick_debug.err" "$(BUILD_DIR)/full_snare_debug.err" "$(BUILD_DIR)/full_tom_debug.err" $(if $(PATTERN_ROUTE),--route "$(PATTERN_ROUTE)") $(PATTERN_ARGS)
