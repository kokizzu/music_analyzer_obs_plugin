...
DETECTION_ACCURACY_MAESTRO_REAL_MANIFEST_ARG = $(if $(wildcard $(MAESTRO_REAL_SAMPLE_DIR)/maestro-v3.0.0.csv),--maestro-real-manifest "$(MAESTRO_REAL_SAMPLE_DIR)/maestro-v3.0.0.csv") $(DETECTION_ACCURACY_MAESTRO_REAL_ATTRIBUTE_ARG) $(DETECTION_ACCURACY_INDEPENDENT_PIANO_STATE_ARG) $(DETECTION_ACCURACY_INDEPENDENT_PIANO_STABILITY_ARG) $(DETECTION_ACCURACY_INDEPENDENT_PIANO_EXACT_FALLBACK_ARG)
...
DETECTION_ACCURACY_MDB_DRUMS_GATE_ARG += $(DETECTION_ACCURACY_BABYSLAKH_DRUMS_GATE_ARG) $(DETECTION_ACCURACY_BABYSLAKH_CALIBRATION_AUDIT_ARG) $(DETECTION_ACCURACY_BABYSLAKH_STATUS_ARGS)
...
update-detection-accuracy-report-cached: scripts/write_detection_accuracy_report.py
	...
	@for path in "$(GUITAR_CHORD_PRIMARY_DISPLAY_AUDIT)" "$(GUITAR_CHORD_TONE_RECOVERY_AUDIT)" "$(URMP_GOOD_SOUNDS_SAX_SHARED_PATTERN_REPORT)" "$(URMP_BASS_TIMING_AUDIT)" "$(OCTAVE_CORRECTION_CROSS_CORPUS_AUDIT)" "$(DOMINANT_SEVENTH_EXTENSION_AUDIT)" "$(GLOBAL_CHORD_CONFIDENCE_AUDIT)" "$(SAME_ROOT_GUITAR_QUALITY_AUDIT)" "$(OWNER_CLASSIFIER_LOCO_AUDIT)" "$(OWNER_CLASSIFIER_QUALITY_LOCO_AUDIT)" "$(DRUM_PRIMARY_LOCO_AUDIT)" "$(DRUM_FALSE_POSITIVE_CAP_AUDIT)" "$(MDB_FULL_MIX_FALSE_POSITIVE_CAP_AUDIT)" "$(MDB_FULL_MIX_COMPETING_ACTIVE_CONTEXT_AUDIT)" "$(DRUM_FALSE_POSITIVE_CONTEXT_AUDIT)" "$(CHORD_PRIMARY_COMPONENT_AUDIT)" "$(INDEPENDENT_PIANO_EXACT_CHORD_FALLBACK_AUDIT)" "$(POLYPHONIC_CANDIDATE_CAPACITY_AUDIT)" "$(HARMONIC_PRODUCT_OCTAVE_AUDIT)"; do test -s "$$path" || { printf '%s\n' "missing cached accuracy audit: $$path"; exit 2; }; done
...
.PHONY: measure-maps-piano-chord-state-cached measure-maestro-real-chord-state-cached analyze-independent-piano-chord-stability-cached test-summarize-piano-chord-state-audit
...
.PHONY: install-beat-this-diagnostic test-measure-beat-this-bpm test-measure-beat-this-rolling-bpm measure-beat-this-gtzan-rhythm measure-beat-this-ballroom measure-beat-this-filobass measure-beat-this-rolling-ballroom measure-beat-this-rolling-filobass summarize-beat-this-gtzan-rhythm summarize-beat-this-real-tempo summarize-beat-this-rolling-tempo
