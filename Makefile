...
update-detection-accuracy-report-cached: scripts/write_detection_accuracy_report.py
	...
	@for path in "$(GUITAR_CHORD_PRIMARY_DISPLAY_AUDIT)" "$(GUITAR_CHORD_TONE_RECOVERY_AUDIT)" "$(URMP_GOOD_SOUNDS_SAX_SHARED_PATTERN_REPORT)" "$(URMP_BASS_TIMING_AUDIT)" "$(OCTAVE_CORRECTION_CROSS_CORPUS_AUDIT)" "$(DOMINANT_SEVENTH_EXTENSION_AUDIT)" "$(GLOBAL_CHORD_CONFIDENCE_AUDIT)" "$(SAME_ROOT_GUITAR_QUALITY_AUDIT)" "$(OWNER_CLASSIFIER_LOCO_AUDIT)" "$(OWNER_CLASSIFIER_QUALITY_LOCO_AUDIT)" "$(DRUM_PRIMARY_LOCO_AUDIT)" "$(DRUM_FALSE_POSITIVE_CAP_AUDIT)" "$(MDB_FULL_MIX_FALSE_POSITIVE_CAP_AUDIT)" "$(MDB_FULL_MIX_COMPETING_ACTIVE_CONTEXT_AUDIT)" "$(DRUM_FALSE_POSITIVE_CONTEXT_AUDIT)" "$(CHORD_PRIMARY_COMPONENT_AUDIT)" "$(INDEPENDENT_PIANO_EXACT_CHORD_FALLBACK_AUDIT)" "$(PIANO_CHORD_CONFIRMATION_AUDIT)" "$(PIANO_CHORD_CONFIRM3_AUDIT)" "$(PIANO_CHORD_TONE018_AUDIT)" "$(PIANO_CHORD_MARGIN060_AUDIT)" "$(MDB_RIM_COVERAGE_AUDIT)" "$(FSD50K_RIM_METADATA_AUDIT)" "$(POLYPHONIC_CANDIDATE_CAPACITY_AUDIT)" "$(HARMONIC_PRODUCT_OCTAVE_AUDIT)"; do test -s "$$path" || { printf '%s\n' "missing cached accuracy audit: $$path"; exit 2; }; done
...
.PHONY: measure-maps-piano-chord-state-cached measure-maestro-real-chord-state-cached analyze-independent-piano-chord-stability-cached test-summarize-piano-chord-state-audit audit-piano-chord-confirmation audit-piano-chord-confirm3 audit-piano-chord-tone018 audit-piano-chord-margin060 test-audit-piano-chord-confirmation
...
.PHONY: install-beat-this-diagnostic test-measure-beat-this-bpm test-measure-beat-this-rolling-bpm measure-beat-this-gtzan-rhythm measure-beat-this-ballroom measure-beat-this-filobass measure-beat-this-rolling-ballroom measure-beat-this-rolling-filobass measure-beat-this-continuous-ballroom measure-beat-this-continuous-filobass summarize-beat-this-gtzan-rhythm summarize-beat-this-real-tempo summarize-beat-this-rolling-tempo
