...
.PHONY: find-real-note-row-confusion-patterns find-real-note-practical-row-confusion-patterns evaluate-real-note-display-shadow measure-real-note-attribute-rule
...
find-drum-full-attribute-patterns: $(BUILD_DIR)/drum_full_attribute_rows.tsv scripts/find_drum_attribute_patterns.py scripts/analyze_drum_primary_debug.py
	$(PYTHON) scripts/find_drum_attribute_patterns.py "$(BUILD_DIR)/drum_full_attribute_rows.tsv" $(if $(PATTERN_ROUTE),--route "$(PATTERN_ROUTE)") $(or $(PATTERN_ARGS),$(MEASURE_DRUM_FULL_PATTERN_ARGS))
...
find-drum-spread-exact-attribute-patterns: $(BUILD_DIR)/analyzer_drum_samples scripts/find_drum_attribute_patterns.py scripts/analyze_drum_primary_debug.py
	...
	@if [ -f "$(DRUM_SPREAD_EXACT_ATTRIBUTE_ROWS)" ]; then $(PYTHON) scripts/find_drum_attribute_patterns.py "$(DRUM_SPREAD_EXACT_ATTRIBUTE_ROWS)" $(if $(PATTERN_ROUTE),--route "$(PATTERN_ROUTE)") $(PATTERN_ARGS); fi
...
find-drum-full-exact-attribute-patterns: $(BUILD_DIR)/analyzer_drum_samples scripts/find_drum_attribute_patterns.py scripts/analyze_drum_primary_debug.py
	...
	$(PYTHON) scripts/find_drum_attribute_patterns.py "$(DRUM_FULL_EXACT_ATTRIBUTE_ROWS)" $(if $(PATTERN_ROUTE),--route "$(PATTERN_ROUTE)") $(or $(PATTERN_ARGS),$(MEASURE_DRUM_FULL_PATTERN_ARGS))
...
find-protected-drum-primary-attribute-patterns: scripts/find_drum_attribute_patterns.py scripts/analyze_drum_primary_debug.py
	...
	@set --; for path in $(DRUM_PROTECTED_PRIMARY_ATTRIBUTE_INPUTS); do if [ -f "$$path" ]; then set -- "$$@" "$$path"; fi; done; if [ "$$#" -eq 0 ]; then printf '%s\n' "protected drum primary pattern candidates: skipped; no attribute rows"; else $(PYTHON) scripts/find_drum_attribute_patterns.py "$$@" $(if $(PATTERN_ROUTE),--route "$(PATTERN_ROUTE)") $(PATTERN_ARGS); fi
...
find-real-note-practical-row-confusion-patterns: $(BUILD_DIR)/real_note_full_mix_attributes.tsv scripts/find_real_note_attribute_patterns.py
	$(PYTHON) scripts/find_real_note_attribute_patterns.py "$(BUILD_DIR)/real_note_full_mix_attributes.tsv" $(if $(PATTERN_BUCKET),--bucket "$(PATTERN_BUCKET)") --bucket-status row_confusion --include-row-context $(REAL_NOTE_RUNTIME_ROW_CONFUSION_EXCLUDES) $(or $(PATTERN_ARGS),$(MEASURE_REAL_NOTE_PRACTICAL_ROW_CONFUSION_PATTERN_ARGS))
...
find-real-note-visual-row-confusion-patterns: $(BUILD_DIR)/real_note_full_mix_attributes.tsv scripts/find_real_note_attribute_patterns.py
	$(PYTHON) scripts/find_real_note_attribute_patterns.py "$(BUILD_DIR)/real_note_full_mix_attributes.tsv" $(if $(PATTERN_BUCKET),--bucket "$(PATTERN_BUCKET)") --bucket-status visual_row_confusion --include-row-context $(REAL_NOTE_RUNTIME_ROW_CONFUSION_EXCLUDES) $(or $(PATTERN_ARGS),$(MEASURE_REAL_NOTE_PRACTICAL_ROW_CONFUSION_PATTERN_ARGS))
...
$(BUILD_DIR)/guitar_chord_mix_attributes.tsv: $(BUILD_DIR)/analyzer_guitarset scripts/prepare_hf_guitar_chord_mix.py scripts/build_sharded_tsv.sh | $(BUILD_DIR)
...
$(BUILD_DIR)/guitar_chord_mix_attributes.shard-%.tsv: FORCE $(BUILD_DIR)/analyzer_guitarset scripts/prepare_hf_guitar_chord_mix.py | $(BUILD_DIR)
	@if [ ! -f "$(GUITAR_CHORD_MIX_SAMPLE_DIR)/manifest.tsv" ] || [ "scripts/prepare_hf_guitar_chord_mix.py" -nt "$(GUITAR_CHORD_MIX_SAMPLE_DIR)/manifest.tsv" ]; then $(MAKE) prepare-guitar-chord-mix-samples; fi
...
$(GUITARSET_ATTRIBUTE_TSV): $(BUILD_DIR)/analyzer_guitarset scripts/build_sharded_tsv.sh | $(BUILD_DIR)
...
$(BUILD_DIR)/guitarset_attributes.shard-%.tsv: FORCE $(BUILD_DIR)/analyzer_guitarset | $(BUILD_DIR)
	@if [ ! -f "$(GUITARSET_MANIFEST)" ]; then $(MAKE) prepare-downloaded-guitarset; fi
...
test: $(TEST_BINS) scripts/run_with_duration.sh
	$(RUN_WITH_DURATION) test $(MAKE) $(PARALLEL_TEST_MAKE_JOBS) test-parallel test-detector-samples-parallel test-fret-control test-fixtures-parallel test-real-goal-fixture
