...
update-detection-accuracy-report: $(BUILD_DIR)/real_note_full_mix_attributes.tsv $(GUITAR_CHORD_PRIMARY_DISPLAY_AUDIT) $(GUITAR_CHORD_TONE_RECOVERY_AUDIT) $(URMP_GOOD_SOUNDS_SAX_SHARED_PATTERN_REPORT) $(OCTAVE_CORRECTION_CROSS_CORPUS_AUDIT) $(POLYPHONIC_CANDIDATE_CAPACITY_AUDIT) inspect-harmonic-product-octave-evidence-cached audit-dominant-seventh-extension audit-global-chord-confidence audit-same-root-guitar-quality evaluate-owner-classifier-loco evaluate-owner-classifier-quality-loco evaluate-drum-primary-loco inspect-high-vocal-octave-evidence-cached scripts/write_detection_accuracy_report.py
